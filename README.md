...
| **R6000 / FP4-MMA** | census 已 bank:**RTX PRO 6000 96GB / 880GiB data workspace** PASS,CUDA capability `[12,0]`,vLLM source candidates **200**,P76/P79/P85/P87/P103 native FP4 contracts 全过。**R5-C0/R5-C1/R5-C2A/R5-C2 已过:**CUTLASS/CuTe 暴露 sm120 `mxf4nvf4` block16 E2M1 + UE4M3 scale ABI;CUTLASS 79d native NVF4+UE4M3 grouped GEMM host-reference numeric smoke PASS;MoE shape census 证明需把 92-style `MoEProblemShape/tokens_per_expert` 语义接到 79d-style SM120 执行;R5-C2 已把 selected-expert `tokens_per_expert=[32,64,64,96]` 映射到 per-group M shape 并通过双 schedule host-reference。只 bank ABI/numeric smoke/source-census/selected-expert gate-up smoke,不 bank grouped-MoE speed/default。 |
...
| **E. native kernels / FP4-MMA** | grouped CUDA C++/CUTLASS kernels + FP4-MMA 硅兑现 | R5-C2 selected-expert gate/up numeric smoke 已 banked;下一步 R5-C2B slot-preserving selected-output bridge,仍不宣称 grouped-MoE speed/default |
...
把 `tokens_per_expert=[32,64,64,96]` 映射到 79d per-group M shape,Cooperative/Pingpong 双 schedule host-reference PASS。下一步是 R5-C2B slot-preserving selected-output bridge,
不是宣称已有 grouped-MoE FP4-MMA kernel 或默认速度提升。
后续换机时仍先按
