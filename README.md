...
| **R6000 / FP4-MMA** | census 已 bank:**RTX PRO 6000 96GB / 880GiB data workspace** PASS,CUDA capability `[12,0]`,vLLM source candidates **200**,P76/P79/P85/P87/P103 native FP4 contracts 全过。**R5-C0→R5-C2C 已过:**CUTLASS/CuTe 暴露 sm120 `mxf4nvf4` block16 E2M1 + UE4M3 scale ABI;CUTLASS 79d native NVF4+UE4M3 grouped GEMM host-reference numeric smoke PASS;MoE shape census 证明需把 92-style `MoEProblemShape/tokens_per_expert` 语义接到 79d-style SM120 执行;R5-C2 selected-expert gate/up 双 schedule PASS;**R5-C2C 已捕获真实 CUTLASS D/ref row digests 并按 R5-C2B inverse-order scatter 到 `[T, top_k, N_gateup]`。R5-C3A trace-only 正速度信号:gate/up prefill shape `8x256x1024x2048` FP4-MMA **0.0206ms/416 TFLOPS**,同形状 BF16 `bmm` **0.0435ms/198 TFLOPS**,约 **2.11×**。** 只 bank ABI/numeric/real-D-row scatter smoke;R5-C3A 仍不 bank epilogue kernel、SwiGLU/down、full MoE speed、decode TPS 或 default。 |
...
| **E. native kernels / FP4-MMA** | grouped CUDA C++/CUTLASS kernels + FP4-MMA 硅兑现 | R5-C2C real D-row slot scatter smoke 已 banked;R5-C3A gate/up prefill trace 出现约 **2.11×** vs BF16 bmm 的正信号;下一步 R5-C3B 补 SwiGLU/down/weighted-sum numeric parity,仍不宣称 full MoE speed/default |
