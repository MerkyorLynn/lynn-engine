...
| **R6000 / FP4-MMA** | census 已 bank:**RTX PRO 6000 96GB / 880GiB data workspace** PASS,CUDA capability `[12,0]`,vLLM source candidates **200**,P76/P79/P85/P87/P103 native FP4 contracts 全过。**R5-C0→R5-C3B 已过:**R5-C2C 捕获真实 CUTLASS D/ref row digests 并按 R5-C2B inverse-order scatter 到 `[T, top_k, N_gateup]`;R5-C3A trace-only 正速度信号:gate/up prefill shape `8x256x1024x2048` FP4-MMA **0.0206ms/416 TFLOPS**,同形状 BF16 `bmm` **0.0435ms/198 TFLOPS**,约 **2.11×**;**R5-C3B 已捕获完整 D/ref row values + exact bits,scatter max_abs=0,host-SwiGLU checksum 双 schedule 一致。** 只 bank ABI/numeric/value materialization;仍不 bank down、full MoE speed、decode TPS 或 default。 |
...
| **E. native kernels / FP4-MMA** | grouped CUDA C++/CUTLASS kernels + FP4-MMA 硅兑现 | R5-C3B gate/up value materialization 已 banked;下一步 R5-C3C 补 down projection + weighted top-k numeric parity,仍不宣称 full MoE speed/default |
