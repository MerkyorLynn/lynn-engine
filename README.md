...
- **P2-A..P2-M 证据:** 单 expert gate/up 输 BF16;routed active path M64=23.83ms/layer;P2-F engine opt-in mode M64=20.23ms/layer;P2-H/P2-I selected full prefill 继续 numeric/no-shadow/speed 通过;P2-J trace 显示 `chunk_gated_delta_with_state` 占 linear-attn prefill traced wall 71-76%;P2-KA 反证逐 token 复用 decode recurrent kernel;P2-KB/P2-L 接通 block linear-attn;P2-M layers 0-3 selected full prefill 继续过关。
- **下一关:** P2-N wider layer coverage + RC/server smoke;dense/MoE M>1 若要追平并超过 BF16,最终要 native FP4-MMA/CUTLASS-style bridge。
