...
| **R6000 / FP4-MMA** | census 已 bank:**RTX PRO 6000 96GB / 880GiB data workspace** PASS,CUDA capability `[12,0]`,vLLM source candidates **200**,P76/P79/P85/P87/P103 native FP4 contracts 全过。**R5-C0/R5-C1 已过:**CUTLASS/CuTe 暴露 sm120 `mxf4nvf4` block16 E2M1 + UE4M3 scale ABI,且 CUTLASS 79d native NVF4+UE4M3 grouped GEMM host-reference numeric smoke PASS;只 bank ABI/numeric smoke,不 bank grouped-MoE kernel/speed/default。 |
...
| **E. native kernels / FP4-MMA** | grouped CUDA C++/CUTLASS kernels + FP4-MMA 硅兑现 | R5-C1 CUTLASS numeric smoke 已 banked;下一步 R5-C2 selected expert gate/up numeric smoke,仍不宣称 grouped-MoE speed/default |
...
下一步是 R5-C2 selected expert gate/up numeric smoke,不是宣称已有 grouped-MoE FP4-MMA kernel 或默认速度提升。
后续换机时仍先按
