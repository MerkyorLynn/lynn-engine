...
| **当前瓶颈** | P4C profile:gate/up **151.67us(63.8%)**,down **86.21us(36.2%)**;gate/up shape sweep 已找到 `tile_inter=2,threads=128`:**91.41us=1.659×**。下一刀把这个 shape 接成 P4C speed candidate。 |

**诚实边界:** Spark sm_121 当前 deliverable decode 仍是 **~44-45 TPS**;P4C 目前只 bank route/numeric/baseline/diagnostic 和 actionable shape-sweep,**不 bank fused speed/default promotion**。Python 只做控制面和验证;追赶 llama.cpp 靠 native kernel,在 FP4-MMA 硅上才完整兑现。
...
- **下一关:** P2-O/P3-B/P3-C/P3-D/P3-E Spark artifacts;P3-D 只 bank opt-in server smoke,P3-E 只 bank RC quality smoke,默认 promotion 与完整 leaderboard 仍需后续 release battery。P4B native zero-shadow 已 bank fail-loud route + single-CTA output-returning numeric reference + 两个 microbench anti-proof(single-CTA 串行慢、multi-CTA 重算 active 更慢)。**P4C active-reuse runtime bridge + speed baseline + component profile + gate/up shape sweep 已 bank:**`fused_zero_shadow_active_reuse_contract` 在 Spark real-runner 路径 PASS;speed baseline 证明 P4C ABI 对 P4A 是 **1.00013×**、output/scratch exact;component profile 显示 gate/up 是 **63.8%** 主耗时,down 是 **36.2%**;shape sweep 找到 `tile_inter=2,threads=128` **91.41us=1.659×** 的 actionable diagnostic。fused speed kernel 与默认 promotion 仍关闭;下一刀先把 gate/up shape 接成 P4C speed candidate,再重跑 P4C baseline/RC。dense/MoE M>1 若要追平并超过 BF16,最终要 native FP4-MMA/CUTLASS-style bridge。
