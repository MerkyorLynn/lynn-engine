...
- **下一关:** P2-O/P3-B/P3-C/P3-D/P3-E Spark artifacts;P3-D 只 bank opt-in server smoke,P3-E 只 bank RC quality smoke,默认 promotion 与完整 leaderboard 仍需后续 release battery。P4B native zero-shadow 已 bank fail-loud route + single-CTA output-returning numeric reference + 两个 microbench anti-proof(single-CTA 串行慢、multi-CTA 重算 active 更慢)。**P4C active-reuse 决策已 bank:**下一版 `fused_zero_shadow_active_reuse_contract` 必须一次计算并复用 `active[top_k,512]`,优先两阶段/CUTLASS-style,不能把重算 active 冒充 fused speed;当前 fused speed kernel 与默认 promotion 仍关闭。dense/MoE M>1 若要追平并超过 BF16,最终要 native FP4-MMA/CUTLASS-style bridge。
