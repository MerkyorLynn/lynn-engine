...
| **packed prefill** | P0.1/P0.2 已过:no-reload `stream_bf16` token-exact,peak **40.28 GiB**,proof prefill **20.75s**;after-release BF16 resident **4.72 GiB**。**P2-O basic 已过:** active-MoE no-reload packed-prefill 3/3 generated-token exact,88.16→28.18 GiB,reload 0 次;但 opt-in prefill **149.62s avg(0.008×)**,所以只 bank 正确性/显存,不 bank 速度。 |
...
| **B. no-reload prefill** | 把 28 GiB 扩到多请求 server,消掉 per-request reload | P2-O basic PASS;rc-mini 需要 prefill slow-mode 加速或 prompt 分块后重跑 |
