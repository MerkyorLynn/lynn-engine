...
>
> **✅ 自有核心已 bank 的最新突破:** 35B NVFP4 服务已把 decode 阶段 BF16 dequant-shadow 从常驻内存中释放,**resident 88→28 GiB(省 ~60 GiB)**,token-exact,TPS 0.998× 无回归;`server/openai_http.py` 已接入 `reload→prefill→release→decode` 服务循环。**P0.1 packed-prefill / zero-reload smoke 已过:**释放 60GiB shadow 后不调用 reload,`LYNN_PACKED_PREFILL_SLOW_MODE=stream_bf16` 从 resident packed NVFP4 逐层临时反量化 prefill,token-exact,peak **40.28 GiB**,decode TPS 42.47 vs baseline 42.97;代价是 proof prefill **20.75s**。下一关:P0.2 resident inventory + P1/P2 真正 **C++/CUDA/Triton batched/grouped kernels**。Python 只做控制面和验证,追赶 llama.cpp 靠 native kernel。
...
- **下一关:** P0.2 projection/shared resident inventory,随后 P1/P2 下沉 C++/CUDA/Triton batched/grouped packed-prefill kernels。
