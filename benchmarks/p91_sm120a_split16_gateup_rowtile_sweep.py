#!/usr/bin/env python3
"""P91: row-tile sweep for the split16 per-16 SM120a FP4 gate/up kernel.

P90 proved a real full-K 8-row gate/up tile. P91 keeps the exact same split16
math but widens the kernel to multiple 8-row tiles in one launch:

* one real routed expert;
* 8/16/32 consecutive gate rows and the matching up rows;
* full hidden K=2048;
* current Lynn-native per-16 activation/weight scales;
* SM120a blockscaled FP4 MMA with two K16 passes per K32 tile.

This remains a contract/performance-shape probe, not a production backend. The
goal is to see whether row-tile parallelism starts amortizing the P90 launch and
atomic overhead while preserving exactness.
"""
...
torch::Tensor p91_split16_gateup_kernel(
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("split16_gateup_kernel", &p91_split16_gateup_kernel,
        "P91 split16 per-16 SM120a FP4 gate/up row-tile sweep kernel");
}
"""
...
__global__ void p91_split16_gateup_kernel_impl(
  ...
  const int row_base = inter_offset + tile * kRows;
  float* gate_out = out + tile * kRows;
  float* up_out = out + row_count + tile * kRows;
  ...
  for (int k_offset = 0; k_offset < kHidden; k_offset += 32) {
    ...
    float u0[4];
    float u1[4];
    run_mma_group(act, weight, row_base, k_offset, packed_stride_m, 0, lane, scale_byte, g0);
    run_mma_group(act, weight, row_base, k_offset, packed_stride_m, 1, lane, scale_byte, g1);
    run_mma_group(act, weight, kIntermediate + row_base, k_offset, packed_stride_m, 0, lane, scale_byte, u0);
    run_mma_group(act, weight, kIntermediate + row_base, k_offset, packed_stride_m, 1, lane, scale_byte, u1);
    accumulate_rows(g0, g1, act_scale, weight_scale, weight_global, row_base, k_offset,
                    scale_stride_m, scale_stride_g, lane, gate_out);
    accumulate_rows(u0, u1, act_scale, weight_scale, weight_global, kIntermediate + row_base, k_offset,
...
torch::Tensor p91_split16_gateup_kernel(
  ...
  TORCH_CHECK(row_count == 8 || row_count == 16 || row_count == 32 || row_count == 64, "row_count must be one of {8,16,32,64}");
  TORCH_CHECK(inter_offset >= 0 && inter_offset + row_count <= kIntermediate, "inter_offset/row_count out of bounds");
  ...
  auto out = torch::zeros({2, row_count}, torch::TensorOptions().device(torch::kCUDA).dtype(torch::kFloat32));

  const int blocks = static_cast<int>(row_count / kRows);
  p91_split16_gateup_kernel_impl<<<blocks, 32>>>(
  ...
  TORCH_CHECK(launch_err == cudaSuccess, "p91_split16_gateup_kernel launch failed: ", cudaGetErrorString(launch_err));
  return out;
}
"""
...
def _build_module(build_root: Path, verbose: bool):
    ...
    cpp_path = build_root / "p91_bindings.cpp"
    cu_path = build_root / "p91_kernel.cu"
    cpp_path.write_text(CPP_SOURCE, encoding="utf-8")
    cu_path.write_text(CUDA_SOURCE, encoding="utf-8")
    return load(
        name="lynn_native_p91_sm120a_split16_gateup_rowtile_sweep",
...
def _reference_rows(
    ...
    out = torch.empty((2, row_count), dtype=torch.float32)
...
def main() -> int:
    ...
    ap.add_argument("--build-dir", default="/tmp/lynn_engine_native_build/p91_sm120a_split16_gateup_rowtile_sweep")
    ...
    ap.add_argument("--row-counts", default="8,16,32,64")
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("P91 requires CUDA")
    ...
    for row_count in row_counts:
        def run_tile(row_count: int = row_count) -> torch.Tensor:
        ...
        worst_half = max_idx // row_count
        worst_row = max_idx % row_count
        variants.append(
            {
                "row_count": row_count,
                "blocks": row_count // 8,
                ...
                "rows_per_ms_median": float((2 * row_count) / statistics.median(times_ms)),
                "contract": {
    ...
    best = min(variants, key=lambda x: x["median_ms"])
    best_throughput = max(variants, key=lambda x: x["rows_per_ms_median"])

    result = {
        "schema_version": "lynn-engine-p91-sm120a-split16-gateup-rowtile-sweep-v1",
        "model": args.model,
        "layer": args.layer,
        ...
        "best_throughput": {
            "row_count": best_throughput["row_count"],
            "rows_per_ms_median": best_throughput["rows_per_ms_median"],
            "median_ms": best_throughput["median_ms"],
        },
        "decision": (
            "PASS: all row-count variants preserve the P90 split16 contract; use best_throughput to choose the next P92 shape."
            if all(v["contract"]["pass_with_tolerance"] for v in variants)
            else "FAIL: at least one row-count variant diverged; inspect row tile ownership before widening further."
        ),
    }
