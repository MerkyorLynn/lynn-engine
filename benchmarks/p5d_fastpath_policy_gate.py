#!/usr/bin/env python3
"""P5-D gate: build a speed-gated native FP4 fastpath policy."""
from __future__ import annotations

import argparse
import json
from pathlib import Path
import sys

ROOT = Path(__file__).resolve().parents[1]
sys.path.insert(0, str(ROOT))

from engine.native_fp4_policy import build_policy_from_p5d_fastpath_reports  # noqa: E402


def main() -> int:
    ap = argparse.ArgumentParser()
    ap.add_argument("--reports", required=True, type=Path)
    ap.add_argument("--out", required=True, type=Path)
    ap.add_argument("--min-cosine", type=float, default=0.999)
    ap.add_argument("--max-rel-l2", type=float, default=0.01)
    ap.add_argument("--min-speed-ratio", type=float, default=1.0)
    args = ap.parse_args()

    policy = build_policy_from_p5d_fastpath_reports(
        args.reports,
        min_cosine=args.min_cosine,
        max_rel_l2=args.max_rel_l2,
        min_speed_ratio=args.min_speed_ratio,
    )
    policy["source_reports"] = str(args.reports)
    policy["verdict"] = "PASS" if policy["allowlist"] else "FAIL"
    args.out.parent.mkdir(parents=True, exist_ok=True)
    args.out.write_text(json.dumps(policy, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(json.dumps(policy, ensure_ascii=False, indent=2))
    return 0 if policy["verdict"] == "PASS" else 2


if __name__ == "__main__":
    raise SystemExit(main())
