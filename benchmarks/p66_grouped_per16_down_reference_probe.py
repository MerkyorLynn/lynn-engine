#!/usr/bin/env python3
"""P66: grouped per-16 down-reference ABI probe.

P65 wires the full active-MoE grouped-per16 ABI. P66 isolates the down half:

    inter[top_k, 512] + expert_ids + routing_weights + Lynn per-16 down weight
        -> hidden[2048]

The native function is still a scalar reference today. The value of this probe
is freezing a named replacement point for the future CUTLASS/custom CUDA down
kernel and measuring its parity/speed against the current Triton down path on
real routed decode inputs.
"""
...
def _run_layer(
    ...
    *,
    ...
    def native_reference() -> torch.Tensor:
    ...
    native_ms = _bench(native_reference, warmup, iters)
    return {
        "layer": layer,
        "expert_ids": [int(x) for x in expert_ids.tolist()],
        "top_k": int(expert_ids.numel()),
        "diff_vs_triton_down": _diff(ref, out),
        "triton_down_ms": triton_ms,
        "native_reference_ms": native_ms,
        "native_vs_triton_speedup": triton_ms / native_ms,
    }
...
def main() -> int:
    ...
    os.environ.setdefault("LYNN_NATIVE_CUDA_BUILD_DIR", "/tmp/lynn_engine_native_build/p66_grouped_down")
    ...
    min_cosine = min(c["diff_vs_triton_down"]["cosine"] for c in cases)
    max_rel_l2 = max(c["diff_vs_triton_down"]["rel_l2"] for c in cases)
    result = {
        "schema_version": "lynn-engine-p66-grouped-per16-down-reference-probe-v1",
        "model": args.model,
        "layers": args.layers,
        "prompt": args.prompt,
        "cases": cases,
        "summary": {
            ...
            "mean_native_reference_ms": mean(c["native_reference_ms"] for c in cases),
            "mean_native_vs_triton_speedup": mean(c["native_vs_triton_speedup"] for c in cases),
            "min_cosine_vs_triton": min_cosine,
            "max_rel_l2_vs_triton": max_rel_l2,
        },
        "contract_pass": bool(min_cosine >= 0.999999 and max_rel_l2 <= 0.01),
        "promote_speed": False,
        "decision": (
            "This is a reference ABI for the future grouped per-16 down kernel. "
            "It should not be promoted for speed; replace its scalar inner loop "
            "with CUTLASS/custom CUDA math only after this report stays green."
        ),
    }
    out = Path(args.out)
