...
def _load_candidate_output_file(
    ...
    """Load a precomputed candidate output for one fixture.

    Candidate directories may either mirror fixture filenames or use the same
    stem.  Accepted tensor keys are the exact expected key, `candidate_output`,
    and `output`.
    """
    from safetensors.torch import load_file

    base = Path(candidate_output_dir)
    candidates = [
        base / fixture_file,
        base / Path(fixture_file).name,
        base / f"{Path(fixture_file).stem}.safetensors",
    ]
    candidate_path = next((p for p in candidates if p.exists()), None)
    if candidate_path is None:
        raise FileNotFoundError(
            f"no candidate output found for {fixture_file!r} in {candidate_output_dir}"
        )

    data = load_file(str(candidate_path), device=device)
    for key in (expected_key, "candidate_output", "output", "moe_output", "routed_output"):
        if key in data:
            return data[key].to(dtype), str(candidate_path)
    raise KeyError(
        f"{candidate_path} does not contain one of "
        f"{expected_key!r}, 'candidate_output', 'output', 'moe_output', 'routed_output'"
    )
...
def run_contract(
    ...
    If neither candidate path is specified, runs Triton-vs-stored self-check.
    If candidate_backend is specified, compares live candidate against reference.
    If candidate_output_dir is specified, compares precomputed candidate tensors
    against stored fixture outputs without loading layer weights.
    """
    from safetensors.torch import load_file
    ...
    if candidate_backend and candidate_output_dir:
        print(
            "[p134] ERROR: use either --candidate-backend or --candidate-output-dir, not both",
            flush=True,
        )
        return []
    ...
    if candidate_output_dir:
        print("[p134] Candidate output mode: precomputed safetensors", flush=True)
        print(f"[p134] Candidate outputs: {candidate_output_dir}", flush=True)
    else:
    ...
    is_self_check = (
        ...
        and candidate_output_dir is None
    ) or candidate_backend == "triton_reference"
    expected_key = "routed_output" if routed_only else "moe_output"
    ...
    for entry in manifest["fixtures"]:
        ...
        if expected_key not in fixture_data:
            if routed_only:
                raise KeyError(
                    f"{fixture_file} lacks routed_output; regenerate fixtures with p133 v2"
                )
        ...
        if candidate_output_dir:
            ...
            ref_ms = 0.0
            candidate_out, _candidate_path = _load_candidate_output_file(
            ...
            candidate_ms = 0.0
            metrics = _compute_metrics(expected_output, candidate_out)
        else:
    ...
    if candidate_output_dir:
        mode = f"candidate-output-dir ({candidate_output_dir})"
    elif is_self_check:
        mode = "self-check (Triton vs stored fixture)"
    else:
        mode = f"candidate ({candidate_backend})"
    print(f"  Mode:       {mode}")
    ...
    elif candidate_output_dir and failed_count == 0:
        print(f"\n  VERDICT: Candidate outputs pass all fixtures.")
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        default=None,
        help="Directory of precomputed candidate output safetensors. "
             "Files may mirror fixture names and contain moe_output, routed_output, "
             "candidate_output, or output.",
    )
