#!/usr/bin/env python3
"""P149 · Qwen3.5-9B NVFP4 fast knob sweep.

P148 showed the full 35B fast profile gives a modest 9B speedup but drifts on
some prompts.  This sweep isolates which knob group is responsible before any
9B default is changed.
"""
from __future__ import annotations

import argparse
import json
import time
from pathlib import Path
from typing import Any

from p148_qwen35_9b_nvfp4_fast_profile import (
    BASELINE_ENV,
    FAST_ENV,
    PROMPTS,
    _compare_modes,
    _run_mode,
    _summarize_mode,
)


def _merge(base: dict[str, str], updates: dict[str, str]) -> dict[str, str]:
    out = dict(base)
    out.update(updates)
    return out


def _fast_without(*keys: str) -> dict[str, str]:
    out = dict(FAST_ENV)
    for key in keys:
        out[key] = BASELINE_ENV.get(key, "0")
    return out


def _candidate_modes() -> list[tuple[str, dict[str, str]]]:
    triton_core = {
        "LYNN_LINEAR_ATTN_RECURRENT_BACKEND": "triton_fused_prepare",
        "LYNN_LINEAR_ATTN_RECURRENT_INPLACE": "1",
        "LYNN_LINEAR_ATTN_GQA_RECURRENT": "1",
        "LYNN_LINEAR_ATTN_CONV_BACKEND": "triton_torch_silu",
        "LYNN_QK_NORM_ROPE_BACKEND": "triton_pair",
        "LYNN_RMSNORM_GATED_BACKEND": "triton",
        "LYNN_LINEAR_STATE_UPDATE": "inplace",
    }
    return [
        ("triton_core_no_graph_no_packed", _merge(BASELINE_ENV, triton_core)),
        ("native_lm_head_only", _merge(BASELINE_ENV, {"LYNN_NATIVE_FP4_LM_HEAD": "1"})),
        (
            "packed_decode_only",
            _merge(BASELINE_ENV, {"LYNN_PACKED_DECODE": "1", "LYNN_PACKED_DECODE_PREPARE_NATIVE": "1"}),
        ),
        ("native_inproj_only", _merge(BASELINE_ENV, {"LYNN_LINEAR_ATTN_INPROJ_FUSED_NATIVE_FP4": "1"})),
        (
            "linear_graph_only",
            _merge(
                BASELINE_ENV,
                {
                    "LYNN_LINEAR_STATE_UPDATE": "inplace",
                    "LYNN_LINEAR_BLOCK_GRAPH": "1",
                    "LYNN_LINEAR_BLOCK_GRAPH_REUSE": "1",
                    "LYNN_LINEAR_BLOCK_GRAPH_PREWARM": "1",
                },
            ),
        ),
        (
            "fast_no_graph",
            _fast_without(
                "LYNN_LINEAR_BLOCK_GRAPH",
                "LYNN_LINEAR_BLOCK_GRAPH_REUSE",
                "LYNN_LINEAR_BLOCK_GRAPH_PREWARM",
            ),
        ),
        (
            "fast_no_packed_decode",
            _fast_without(
                "LYNN_PACKED_DECODE",
                "LYNN_PACKED_DECODE_PREPARE_NATIVE",
                "LYNN_LINEAR_ATTN_INPROJ_FUSED_NATIVE_FP4",
            ),
        ),
        ("fast_no_native_lm_head", _fast_without("LYNN_NATIVE_FP4_LM_HEAD")),
        ("full_fast_profile", dict(FAST_ENV)),
    ]


def main() -> int:
    ap = argparse.ArgumentParser(description="Qwen3.5-9B NVFP4 fast knob sweep.")
    ap.add_argument("--model", required=True)
    ap.add_argument("--max-new", type=int, default=128)
    ap.add_argument("--max-seq-len", type=int, default=4096)
    ap.add_argument("--prompt", action="append", default=None)
    ap.add_argument("--out", required=True)
    args = ap.parse_args()

    prompts = args.prompt or PROMPTS
    max_new_values = [args.max_new]
    report: dict[str, Any] = {
        "schema": "lynn-qwen35-9b-nvfp4-fast-knob-sweep-v1",
        "created": time.strftime("%Y-%m-%dT%H:%M:%S"),
        "model": args.model,
        "max_new": args.max_new,
        "max_seq_len": args.max_seq_len,
        "prompts": prompts,
        "candidates": [],
    }

    baseline = _run_mode(
        model=args.model,
        label="baseline_conservative",
        env=BASELINE_ENV,
        max_new_values=max_new_values,
        prompts=prompts,
        max_seq_len=args.max_seq_len,
    )
    report["baseline"] = baseline
    report["baseline_summary"] = _summarize_mode(baseline)

    for label, env in _candidate_modes():
        try:
            mode = _run_mode(
                model=args.model,
                label=label,
                env=env,
                max_new_values=max_new_values,
                prompts=prompts,
                max_seq_len=args.max_seq_len,
            )
            comparison = _compare_modes(baseline, mode)
            row = {
                "label": label,
                "status": "OK",
                "summary": _summarize_mode(mode),
                "comparison": comparison,
                "env": env,
            }
            print(
                f"[p149] {label}: exact={comparison['exact_count']}/{comparison['total']} "
                f"speedup_mean={comparison['speedup_mean']}",
                flush=True,
            )
        except Exception as exc:  # keep sweeping other knobs
            row = {
                "label": label,
                "status": "ERROR",
                "error_type": type(exc).__name__,
                "error": str(exc),
                "env": env,
            }
            print(f"[p149] {label}: ERROR {type(exc).__name__}: {exc}", flush=True)
        report["candidates"].append(row)

    exact_candidates = [
        c for c in report["candidates"]
        if c.get("status") == "OK" and c.get("comparison", {}).get("all_exact")
    ]
    exact_candidates.sort(
        key=lambda c: c.get("comparison", {}).get("speedup_mean") or 0.0,
        reverse=True,
    )
    report["best_exact_candidate"] = exact_candidates[0]["label"] if exact_candidates else None
    report["verdict"] = "HAS_EXACT_FAST_CANDIDATE" if exact_candidates else "NO_EXACT_FAST_CANDIDATE"

    out_path = Path(args.out)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_path.write_text(json.dumps(report, indent=2, ensure_ascii=False) + "\n")
    print(json.dumps({
        "verdict": report["verdict"],
        "best_exact_candidate": report["best_exact_candidate"],
        "baseline_summary": report["baseline_summary"],
        "out": str(out_path),
    }, indent=2, ensure_ascii=False))
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
