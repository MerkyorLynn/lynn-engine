#!/usr/bin/env python3
"""P92: full 512-row split16 per-16 SM120a FP4 gate/up expert probe.

P91 found that 64-row row tiles preserve exactness and amortize overhead. P92
pushes the same kernel shape to the full expert gate/up projection:
...
This remains a probe, not a runtime backend. Passing P92 means the current
Lynn-native artifact can feed a complete native-FP4 gate/up expert sub-operator.
"""
...
torch::Tensor p92_split16_gateup_kernel(
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("split16_gateup_kernel", &p92_split16_gateup_kernel,
        "P92 full expert split16 per-16 SM120a FP4 gate/up kernel");
}
"""
...
__global__ void p92_split16_gateup_kernel_impl(
...
torch::Tensor p92_split16_gateup_kernel(
  ...
  TORCH_CHECK(row_count == 512, "P92 row_count must be 512");
  ...
  p92_split16_gateup_kernel_impl<<<blocks, 32>>>(
  ...
  TORCH_CHECK(launch_err == cudaSuccess, "p92_split16_gateup_kernel launch failed: ", cudaGetErrorString(launch_err));
  return out;
}
"""
...
def _build_module(build_root: Path, verbose: bool):
    ...
    cpp_path = build_root / "p92_bindings.cpp"
    cu_path = build_root / "p92_kernel.cu"
    cpp_path.write_text(CPP_SOURCE, encoding="utf-8")
    cu_path.write_text(CUDA_SOURCE, encoding="utf-8")
    return load(
        name="lynn_native_p92_sm120a_split16_gateup_full_expert",
...
def main() -> int:
    ...
    ap.add_argument("--build-dir", default="/tmp/lynn_engine_native_build/p92_sm120a_split16_gateup_full_expert")
    ...
    ap.add_argument("--row-count", type=int, default=512)
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("P92 requires CUDA")
    ...
    result = {
        "schema_version": "lynn-engine-p92-sm120a-split16-gateup-full-expert-v1",
        "model": args.model,
        "layer": args.layer,
        ...
        "decision": (
            "PASS: full 512-row gate/up expert split16 kernel preserves the current Lynn-native artifact contract."
            if bool(err.max().item() <= 1e-5)
            else "FAIL: full 512-row gate/up expert split16 kernel diverged; inspect row tile ownership before runtime integration."
        ),
    }
