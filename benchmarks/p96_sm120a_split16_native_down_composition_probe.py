#!/usr/bin/env python3
"""P96: compose P93 split16 gate/up with the fastest P95 native down backend.

P93 proved a production-shaped top-k gate/up backend:

    quantized hidden -> native FP4 gate/up -> inter[top_k, 512]

P95 showed that `native_down_tile1` is 2.14x faster than the production Triton
down kernel while preserving the quantized-activation contract. P96 composes:

    quantized hidden -> P93 native gate/up -> native_down_tile1

and compares the full active-MoE result against the current production active
path. This is still a probe, not a runtime promotion.
"""
...
def main() -> int:
    ...
    ap.add_argument("--build-dir", default="/tmp/lynn_engine_native_build/p96_sm120a_split16_native_down_composition")
    ap.add_argument("--native-build-dir", default="/tmp/lynn_engine_native_build/p96_runtime_native")
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("P96 requires CUDA")
    _prepare_path()
    ...
    result = {
        "schema_version": "lynn-engine-p96-sm120a-split16-native-down-composition-v1",
        "model": args.model,
        "layer": args.layer,
        "prompt": args.prompt,
        "top_k": top_k,
        ...
        "decision": (
            "PASS: P93 native gate/up plus native_down_tile1 composes into a faster full active-MoE candidate; next gate is full-generate parity."
            if contract_pass
