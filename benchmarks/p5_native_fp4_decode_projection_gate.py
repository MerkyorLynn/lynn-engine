#!/usr/bin/env python3
"""P5-B probe: linear-attention decode with opt-in native FP4 projections.

P5-A proved one PackedNVFP4Linear can run through torch._scaled_mm with fused
activation quantization. P5-B asks the next practical question: if decode-time
linear-attention projections use that native path, does layer math remain close
enough to the scalar packed bridge?

This probe does not change engine defaults. It constructs a local subclass whose
default forward backend is `native_scaled_mm`, then passes those objects through
...
def _load_packed_linear(
    ...
    *,
    ...
    return load_packed_nvfp4_linear(
        v8_dir,
        base_key,
        name=short_name,
        device=device,
...
def main() -> int:
    ...
    result: dict[str, Any] = {
        "schema_version": "lynn-engine-p5-native-fp4-decode-projection-gate-v1",
        "v8_model": str(v8_dir),
        "layer": args.layer,
        ...
        "timing_ms": {
            ...
            "native_scaled_mm_decode_linear_attn": _bench(
        ...
        "notes": [
            "P5-B is an opt-in drift gate; engine defaults still use scalar_bridge.",
            "native_scaled_mm quantizes activations to FP4, so native_vs_scalar is allowed more drift than P3 scalar bridge gates.",
        ],
    }
    result["timing_ms"]["native_vs_scalar_speed_ratio"] = (
        result["timing_ms"]["scalar_bridge_decode_linear_attn"]
        / result["timing_ms"]["native_scaled_mm_decode_linear_attn"]
    )
