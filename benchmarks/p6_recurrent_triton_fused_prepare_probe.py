#!/usr/bin/env python3
"""P6-D: Triton recurrent probe with prepare fused into the kernel.

This extends P6-C by making the kernel consume q/k/v/g/beta in the original
decode layout and performing l2norm/scale/head indexing inside the kernel.
"""
...
def _recurrent_fused_prepare_kernel(
    ...
    # q/k/v are original decode layout [B=1, T=1, H, D], contiguous with D fastest.
...
def recurrent_triton_fused_prepare(q, k, v, g, beta, s_prev):
    out = torch.empty((NUM_V_HEADS, HEAD_V_DIM), device=q.device, dtype=torch.float32)
    s_new = torch.empty_like(s_prev)
    _recurrent_fused_prepare_kernel[(NUM_V_HEADS, 4)](
        q.reshape(NUM_V_HEADS, HEAD_K_DIM),
        k.reshape(NUM_V_HEADS, HEAD_K_DIM),
        v.reshape(NUM_V_HEADS, HEAD_V_DIM),
        g.reshape(NUM_V_HEADS),
        beta.reshape(NUM_V_HEADS),
        s_prev.reshape(NUM_V_HEADS, HEAD_K_DIM, HEAD_V_DIM),
        out,
        s_new.reshape(NUM_V_HEADS, HEAD_K_DIM, HEAD_V_DIM),
        BLOCK_K=HEAD_K_DIM,
    ...
    return out.reshape(1, 1, NUM_V_HEADS, HEAD_V_DIM).to(q.dtype), s_new.reshape(1, NUM_V_HEADS, HEAD_K_DIM, HEAD_V_DIM)
...
def main() -> int:
    ...
    fused_out, fused_s = recurrent_triton_fused_prepare(q, k, v, g, beta, s_prev)

    result = {
        "schema_version": "lynn-engine-p6d-recurrent-triton-fused-prepare-probe-v1",
        "device": torch.cuda.get_device_name(args.device),
        "comparisons": {
            "triton_prepared_vs_ref": {"out": _cmp(tri_out, ref_out), "state": _cmp(tri_s, ref_s)},
            "triton_fused_prepare_vs_ref": {"out": _cmp(fused_out, ref_out), "state": _cmp(fused_s, ref_s)},
        },
        "latency_ms": {
            ...
            "triton_fused_prepare": _bench(lambda: recurrent_triton_fused_prepare(q, k, v, g, beta, s_prev), args.warmup, args.iters),
        },
    }
    result["derived"] = {
        ...
        "triton_fused_prepare_vs_ref_speed_ratio": result["latency_ms"]["ref_full"] / result["latency_ms"]["triton_fused_prepare"],
    }
