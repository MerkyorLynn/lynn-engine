...
Runs LynnIncrementalRunner with LYNN_NATIVE_ACTIVE_MOE_BACKEND=packed_pretransposed_graphsafe_v31
...
def main() -> int:
    ...
    print(f"[p143] V3.1 resident P37 probe")
    print(f"[p143] Model: {args.model}")
    print(f"[p143] Graph-on: {args.graph_on}")
    print(f"[p143] Max new tokens: {args.max_new}")
    print()

    # Baseline: Triton (known-good)
    print("[p143] Running BASELINE (triton)...")
    ...
    # Candidate: V3.1
    print("\n[p143] Running CANDIDATE (packed_pretransposed_graphsafe_v31)...")
    candidate = _run_mode(args.model, "graphsafe_v31", "packed_pretransposed_graphsafe_v31", args.max_new, graph_on=args.graph_on)
    for r in candidate:
        print(f"  P{r['prompt_id']}: {r['new_ids'][:8]}... tps={r['decode_tps']:.1f}" if r['decode_tps'] else f"  P{r['prompt_id']}: {r['new_ids'][:8]}...")
    ...
    report = {
        "probe": "p143_resident_moe_graphsafe_p37",
