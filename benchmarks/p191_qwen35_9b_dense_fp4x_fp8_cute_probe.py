...
def main() -> int:
    ...
    ap.add_argument("--sidecar-dir", default=None, help="Optional P192 repack sidecar to load packed weights/scales")
    ...
    if sidecar_dir is not None:
        print(f"[p191] Sidecar: {sidecar_dir}")
    ...
    for layer_id in layers:
        ...
        # For this PoC: test gate_proj only (4096 -> 12288). Prefer the P192
        # sidecar when provided so the probe exercises the same offline repack
        # artifact that resident serving will eventually load.
        weight_source = "model"
        if sidecar_dir is not None:
            sidecar_file = sidecar_dir / f"layer_{layer_id:02d}.safetensors"
            if not sidecar_file.exists():
                print(f"  L{layer_id:02d}: SKIP (missing sidecar {sidecar_file})")
                continue
            side = load_file(str(sidecar_file), device="cuda")
            w_packed = side["gate_proj.weight_packed"]          # [N, K/2]
            w_scale = side["gate_proj.weight_scale"].float()    # [N, K/16]
            w_global = side["gate_proj.weight_global_scale"].float()  # scalar
            weight_source = "p192_sidecar"
        else:
