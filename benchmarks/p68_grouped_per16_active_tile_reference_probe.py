#!/usr/bin/env python3
"""P68: grouped per-16 full active-MoE tile-reference probe.

P66/P67 isolated the down half. P68 composes a complete active expert FFN
candidate under the grouped-per16 ABI:

  gate/up tile-inter reference -> down tile-hidden reference

This is still scalar CUDA reference math. The point is to measure the combined
active-MoE signal on real routed inputs without pretending it is the final
tensor-core grouped kernel or a production runtime default.
"""
...
def _run_layer(
    ...
    *,
    ...
    def native_tile_active() -> torch.Tensor:
    ...
    native = native_tile_active()
    triton_ms = _bench(triton_active, warmup, iters)
    native_ms = _bench(native_tile_active, warmup, iters)
    return {
        "layer": layer,
        ...
        "diff_native_tile_vs_triton": _diff(triton, native),
        "triton_active_ms": triton_ms,
        "native_tile_active_ms": native_ms,
        "native_tile_vs_triton_speedup": triton_ms / native_ms,
    }
...
def main() -> int:
    ...
    os.environ.setdefault("LYNN_NATIVE_CUDA_BUILD_DIR", "/tmp/lynn_engine_native_build/p68_grouped_active_tile")
    ...
    min_cosine = min(c["diff_native_tile_vs_triton"]["cosine"] for c in cases)
    max_rel_l2 = max(c["diff_native_tile_vs_triton"]["rel_l2"] for c in cases)
    result = {
        "schema_version": "lynn-engine-p68-grouped-per16-active-tile-reference-probe-v1",
        "model": args.model,
        "layers": args.layers,
        "prompt": args.prompt,
        "tile_inter": args.tile_inter,
        ...
        "summary": {
            ...
            "mean_native_tile_active_ms": mean(c["native_tile_active_ms"] for c in cases),
            "mean_native_tile_vs_triton_speedup": mean(c["native_tile_vs_triton_speedup"] for c in cases),
        ...
        "decision": (
            "This is a complete active-MoE tiled reference candidate under the "
            "grouped per-16 ABI. It remains a measurement path only; P48/P50 "
            "full-decode drift still blocks runtime default promotion."
        ),
    }
    out = Path(args.out)
