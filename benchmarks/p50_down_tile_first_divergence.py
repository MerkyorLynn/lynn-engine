#!/usr/bin/env python3
"""P50: first-divergence probe for the P48 down-tile backend.

P49 showed the tile-hidden down kernel matches Triton on true decode-state MoE
inputs.  Runtime generate gates still drift, so this probe compares full
decode steps with CUDA graphs disabled:

    baseline:  LYNN_NATIVE_DOWN_BACKEND=triton
    candidate: LYNN_NATIVE_DOWN_BACKEND=cuda_tile

Both paths follow the baseline greedy token stream.  If P50 passes while the
runtime gate fails, the bug is graph/capture/state interaction rather than the
down kernel itself.
"""
...
def _set_common_env(layer_spec: str, tile: int) -> dict[str, str | None]:
    updates = dict(COMMON_ENV)
    updates["LYNN_NATIVE_ACTIVE_MOE_LAYERS"] = layer_spec
    updates["LYNN_NATIVE_DOWN_TILE_HIDDEN"] = str(tile)
...
def _decode_step(
    ...
    old = os.environ.get("LYNN_NATIVE_DOWN_BACKEND")
    os.environ["LYNN_NATIVE_DOWN_BACKEND"] = down_backend
    try:
        ...
        if old is None:
            os.environ.pop("LYNN_NATIVE_DOWN_BACKEND", None)
        else:
            os.environ["LYNN_NATIVE_DOWN_BACKEND"] = old
...
def main() -> int:
    ...
    old_env = _set_common_env(args.layers, args.tile)
    try:
        ...
        for step in range(args.steps):
            ...
            h_c, logits_c, layers_c = _decode_step(runner, state_cand, next_id, "cuda_tile")
            ...
            if first_top1_divergence is None and not row["top1_match"]:
                first_top1_divergence = {
                    ...
                    "cuda_tile_top1": top_c["ids"][0],
                    "triton_margin": top_r["margin"],
                    "cuda_tile_margin": top_c["margin"],
                }
        ...
        result = {
            "schema_version": "lynn-engine-p50-down-tile-first-divergence-v1",
            "model": args.model,
            "prompt": args.prompt,
            "steps": args.steps,
            "tile": args.tile,
    ...
    print(json.dumps({
        "pass": result["pass"],
        "layers_spec": result["layers_spec"],
