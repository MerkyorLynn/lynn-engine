#!/usr/bin/env python3
"""P30: full active-MoE native CUDA extension contract probe.

P28 and P29 validated the two halves independently:
  - gate/up packed NVFP4 -> inter[top_k, 512]
  - down packed NVFP4 + routing -> hidden[2048]

P30 composes both native CUDA extension calls and compares the full active
routed expert output against the production Triton active-MoE path.
"""
...
def _load_native(build_dir: str):
    ...
    return load(
        name="lynn_native_p30_active_moe",
        sources=[
...
def _run_layer(
    ...
    *,
    ...
    def cuda_active() -> torch.Tensor:
        inter = module.gate_up_silu_scalar(
            hidden,
            expert_ids,
            gate_up_packed,
            gate_up_scale,
    ...
    return {
        ...
        "cuda_scalar_active_ms": _bench(cuda_active, warmup, iters),
    }
...
def main() -> int:
    ...
    ap.add_argument("--build-dir", default="/tmp/lynn_engine_native_build/p30_active_moe")
    args = ap.parse_args()
    ...
    for case in cases:
        case["cuda_vs_triton_speedup"] = case["triton_active_ms"] / case["cuda_scalar_active_ms"]
    promote = all(case["diff_vs_triton_ref"]["cosine"] >= 0.999999 for case in cases)
    result = {
        "schema_version": "lynn-engine-p30-native-active-moe-cuda-probe-v1",
        "model": args.model,
        "layers": args.layers,
        ...
        "promote_speed": all(case["cuda_scalar_active_ms"] < case["triton_active_ms"] for case in cases),
        "notes": [
            "This composes P28 gate/up and P29 down into a full native active-MoE scalar path.",
            "It is a contract probe; the next speed path must replace scalar inner loops with grouped native FP4 math.",
        ],
    }
    out = Path(args.out)
