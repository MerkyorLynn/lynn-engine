...
if HAS_TRITON:
    ...
    @triton.jit
    def _grouped_down_weighted_sum_batched_kernel(
        inter_ptr,
        expert_ids_ptr,
        routing_weights_ptr,
        down_packed_ptr,
        ...
        SCALE_STRIDE_M: tl.constexpr,
        SCALE_STRIDE_G: tl.constexpr,
        INTER_STRIDE_T: tl.constexpr,
        INTER_STRIDE_K: tl.constexpr,
        INTER_STRIDE_I: tl.constexpr,
        EID_STRIDE_T: tl.constexpr,
        EID_STRIDE_K: tl.constexpr,
        ROUTE_STRIDE_T: tl.constexpr,
        ...
        for slot in range(0, TOP_K):
            expert = tl.load(expert_ids_ptr + token * EID_STRIDE_T + slot * EID_STRIDE_K)
            route = tl.load(routing_weights_ptr + token * ROUTE_STRIDE_T + slot * ROUTE_STRIDE_K).to(tl.float32)
            slot_acc = tl.zeros((BLOCK_HIDDEN,), dtype=tl.float32)
            for i0 in range(0, INTERMEDIATE, BLOCK_INTER):
                cols = i0 + inter_offsets
                col_mask = cols < INTERMEDIATE
                ...
                scale_cols = cols // 16
                inter = tl.load(
                    inter_ptr + token * INTER_STRIDE_T + slot * INTER_STRIDE_K + cols * INTER_STRIDE_I,
                    mask=col_mask,
                    other=0.0,
                ).to(tl.float32)
                packed_offsets = (
        ...
        tl.store(out_ptr + token * OUT_STRIDE_T + rows * OUT_STRIDE_H, acc.to(tl.bfloat16), mask=row_mask)
...
def nvfp4_grouped_down_weighted_sum_batched(
    inter: torch.Tensor,
    expert_ids: torch.Tensor,
    routing_weights: torch.Tensor,
    down_packed: torch.Tensor,
    ...
    *,
    ...
    """Compute weighted top-k down projection for a token batch in one launch.

    ``inter`` is ``[tokens, top_k, 512]``. This preserves the single-token
    weighted-sum math while removing the Python ``for token`` launch loop used
    by the P3-A contract-shaped packed-prefill path.
    """
    _require_triton()
    if inter.ndim != 3 or inter.shape[2] != INTERMEDIATE_SIZE:
        raise ValueError(f"inter must be [tokens, top_k, 512], got {tuple(inter.shape)}")
    if expert_ids.shape != inter.shape[:2] or routing_weights.shape != inter.shape[:2]:
        raise ValueError(
            "expert_ids/routing_weights must match inter [tokens, top_k], got "
            f"inter={tuple(inter.shape)} expert_ids={tuple(expert_ids.shape)} "
    ...
    tokens = int(inter.shape[0])
    expert_ids = expert_ids.to(device=inter.device, dtype=torch.int32).contiguous()
    routing_weights = routing_weights.to(device=inter.device, dtype=torch.float32).contiguous()
    inter = inter.contiguous()
    if out is None:
        out = torch.empty((tokens, HIDDEN_SIZE), device=inter.device, dtype=torch.bfloat16)
    else:
        if out.shape != (tokens, HIDDEN_SIZE):
            raise ValueError(f"out must be [{tokens}, {HIDDEN_SIZE}], got {tuple(out.shape)}")
        if out.device != inter.device or out.dtype != torch.bfloat16:
            raise ValueError("out must be a bfloat16 tensor on the same device as inter")
    grid = (tokens, triton.cdiv(HIDDEN_SIZE, block_hidden))
    _grouped_down_weighted_sum_batched_kernel[grid](
        inter,
        expert_ids,
        routing_weights,
        down_packed.contiguous(),
