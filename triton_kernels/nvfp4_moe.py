...
if HAS_TRITON:
    ...
    _SP01_GATEUP_CONFIGS = [
        ...
        triton.Config({"BLOCK_INTER": 8, "BLOCK_HIDDEN": 128}, num_warps=4, num_stages=2),
        triton.Config({"BLOCK_INTER": 8, "BLOCK_HIDDEN": 256}, num_warps=4, num_stages=2),
        triton.Config({"BLOCK_INTER": 8, "BLOCK_HIDDEN": 256}, num_warps=8, num_stages=3),
        triton.Config({"BLOCK_INTER": 16, "BLOCK_HIDDEN": 64}, num_warps=2, num_stages=2),
        triton.Config({"BLOCK_INTER": 16, "BLOCK_HIDDEN": 128}, num_warps=4, num_stages=2),
        triton.Config({"BLOCK_INTER": 16, "BLOCK_HIDDEN": 256}, num_warps=4, num_stages=3),
        triton.Config({"BLOCK_INTER": 32, "BLOCK_HIDDEN": 64}, num_warps=4, num_stages=2),
        triton.Config({"BLOCK_INTER": 32, "BLOCK_HIDDEN": 128}, num_warps=4, num_stages=2),
        triton.Config({"BLOCK_INTER": 32, "BLOCK_HIDDEN": 256}, num_warps=8, num_stages=3),
        triton.Config({"BLOCK_INTER": 64, "BLOCK_HIDDEN": 64}, num_warps=4, num_stages=2),
        triton.Config({"BLOCK_INTER": 64, "BLOCK_HIDDEN": 128}, num_warps=4, num_stages=2),
        triton.Config({"BLOCK_INTER": 64, "BLOCK_HIDDEN": 256}, num_warps=8, num_stages=3),
        triton.Config({"BLOCK_INTER": 128, "BLOCK_HIDDEN": 128}, num_warps=4, num_stages=2),
        triton.Config({"BLOCK_INTER": 128, "BLOCK_HIDDEN": 256}, num_warps=8, num_stages=2),
    ]

    @triton.autotune(configs=_SP01_GATEUP_CONFIGS, key=["HIDDEN", "INTERMEDIATE"])
    @triton.jit
    def _grouped_gate_up_silu_kernel_sp01_autotuned(
        x_ptr,
        expert_ids_ptr,
        gate_up_packed_ptr,
        gate_up_scale_ptr,
    ...
    _SP01_DOWN_CONFIGS = [
        ...
        triton.Config({"BLOCK_HIDDEN": 8, "BLOCK_INTER": 512}, num_warps=4, num_stages=2),
        triton.Config({"BLOCK_HIDDEN": 16, "BLOCK_INTER": 64}, num_warps=2, num_stages=2),
        triton.Config({"BLOCK_HIDDEN": 16, "BLOCK_INTER": 128}, num_warps=2, num_stages=2),
        triton.Config({"BLOCK_HIDDEN": 16, "BLOCK_INTER": 256}, num_warps=4, num_stages=2),
        triton.Config({"BLOCK_HIDDEN": 16, "BLOCK_INTER": 512}, num_warps=4, num_stages=3),
        triton.Config({"BLOCK_HIDDEN": 32, "BLOCK_INTER": 64}, num_warps=2, num_stages=2),
        triton.Config({"BLOCK_HIDDEN": 32, "BLOCK_INTER": 128}, num_warps=4, num_stages=2),
        triton.Config({"BLOCK_HIDDEN": 32, "BLOCK_INTER": 256}, num_warps=4, num_stages=2),
        triton.Config({"BLOCK_HIDDEN": 32, "BLOCK_INTER": 512}, num_warps=8, num_stages=3),
        triton.Config({"BLOCK_HIDDEN": 64, "BLOCK_INTER": 128}, num_warps=4, num_stages=2),
        triton.Config({"BLOCK_HIDDEN": 64, "BLOCK_INTER": 256}, num_warps=4, num_stages=2),
        triton.Config({"BLOCK_HIDDEN": 64, "BLOCK_INTER": 512}, num_warps=8, num_stages=3),
        triton.Config({"BLOCK_HIDDEN": 128, "BLOCK_INTER": 256}, num_warps=4, num_stages=2),
        triton.Config({"BLOCK_HIDDEN": 128, "BLOCK_INTER": 512}, num_warps=8, num_stages=2),
    ]

    @triton.autotune(configs=_SP01_DOWN_CONFIGS, key=["TOP_K", "HIDDEN", "INTERMEDIATE"])
    @triton.jit
    def _grouped_down_weighted_sum_kernel_sp01_autotuned(
        inter_ptr,
        expert_ids_ptr,
        routing_weights_ptr,
        down_packed_ptr,
...
def nvfp4_grouped_gate_up_silu_sp01_autotuned(
    x: torch.Tensor,
    expert_ids: torch.Tensor,
    gate_up_packed: torch.Tensor,
    gate_up_scale: torch.Tensor,
    ...
    """P_SP_01 sm_121 autotuned variant — block sizes / warps / stages picked by Triton.

    Same math as `nvfp4_grouped_gate_up_silu`; the kernel body is identical but
    wrapped with @triton.autotune across 17 (BLOCK_INTER, BLOCK_HIDDEN,
    num_warps, num_stages) candidates. First call on Spark sm_121 pays a
    one-time autotune cost (~50-300 ms); subsequent calls reuse the picked
    config in-process.
    """
    _require_triton()
    if x.ndim != 1 or x.numel() != HIDDEN_SIZE:
    ...
    expert_ids = expert_ids.to(device=x.device, dtype=torch.int32).contiguous()
    inter = torch.empty((expert_ids.numel(), INTERMEDIATE_SIZE), device=x.device, dtype=torch.bfloat16)
    grid = lambda meta: (expert_ids.numel(), triton.cdiv(INTERMEDIATE_SIZE, meta["BLOCK_INTER"]))
    _grouped_gate_up_silu_kernel_sp01_autotuned[grid](
        x.contiguous(),
        expert_ids,
        gate_up_packed.contiguous(),
        gate_up_scale.contiguous(),
...
def nvfp4_grouped_down_weighted_sum_sp01_autotuned(
    inter: torch.Tensor,
    expert_ids: torch.Tensor,
    routing_weights: torch.Tensor,
    down_packed: torch.Tensor,
    ...
    """P_SP_01 sm_121 autotuned variant — see gate_up counterpart for context."""
    _require_triton()
    if inter.ndim != 2 or inter.shape[1] != INTERMEDIATE_SIZE:
        raise ValueError(f"inter must be [top_k, 512], got {tuple(inter.shape)}")
    if down_packed.ndim != 3 or down_scale.ndim != 3:
    ...
    if expert_ids.numel() != inter.shape[0] or routing_weights.numel() != inter.shape[0]:
        raise ValueError("expert_ids/routing_weights must match inter top_k")
    out = torch.empty((HIDDEN_SIZE,), device=inter.device, dtype=torch.bfloat16)
    grid = lambda meta: (triton.cdiv(HIDDEN_SIZE, meta["BLOCK_HIDDEN"]),)
    _grouped_down_weighted_sum_kernel_sp01_autotuned[grid](
        inter.contiguous(),
        expert_ids,
        routing_weights,
        down_packed.contiguous(),
