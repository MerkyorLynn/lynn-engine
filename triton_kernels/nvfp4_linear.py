...
if HAS_TRITON:
    ...
    @triton.jit
    def _nvfp4_batched_matvec_kernel(
        x_ptr,
        packed_ptr,
        scale_ptr,
        global_scale_ptr,
        ...
        for n0 in range(0, N, BLOCK_N):
            ...
            x = tl.load(
                x_ptr + token * x_stride_t + cols,
                mask=(token < T) & col_mask,
                other=0.0,
            ).to(tl.float32)
            acc += tl.sum(w_fp4 * (scale / global_scale) * x[None, :], axis=1)
...
def nvfp4_batched_matmul_packed(
    x: torch.Tensor,
    weight_packed: torch.Tensor,
    weight_scale: torch.Tensor,
    weight_global_scale: torch.Tensor | None = None,
    *,
    ...
    """Run `x @ weight.T` for M>1 directly from packed NVFP4 weights.

    This Phase-1A bridge is intentionally simple: one Triton launch covers all
    token rows, but each program still computes one token and one output-row
    block. It removes the Python row loop used by the no-reload proof path; it
    is not yet a tensor-core or shared-activation optimized prefill kernel.
    """
    _require_triton()
    if not x.is_cuda or not weight_packed.is_cuda or not weight_scale.is_cuda:
        raise ValueError("x, weight_packed, and weight_scale must be CUDA tensors")
    ...
    grid = (tokens, triton.cdiv(out_features, block_m))
    _nvfp4_batched_matvec_kernel[grid](
        x,
        weight_packed,
        weight_scale,
        weight_global_scale,
