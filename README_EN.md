...
| **R6000 / FP4-MMA** | Census is banked:**RTX PRO 6000 96GB / 880GiB data workspace** PASS, CUDA capability `[12,0]`, vLLM source candidates **200**, and P76/P79/P85/P87/P103 native FP4 contracts all pass. **R5-C0→R5-C3B passed:** R5-C2C captures real CUTLASS D/ref row digests and scatters them through the R5-C2B inverse order into `[T, top_k, N_gateup]`; R5-C3A trace-only positive speed signal: gate/up prefill shape `8x256x1024x2048` FP4-MMA is **0.0206ms / 416 TFLOPS**, same-shape BF16 `bmm` is **0.0435ms / 198 TFLOPS**, about **2.11x**; **R5-C3B captures full D/ref row values plus exact bits, scatter max_abs=0, and matching host-SwiGLU checksums on both schedules.** ABI/numeric/value materialization only; still no down projection, full-MoE speed, decode TPS, or defaults. |
...
| **E. native kernels / FP4-MMA** | Grouped CUDA C++/CUTLASS kernels plus FP4-MMA silicon payoff | R5-C3B gate/up value materialization is banked; next is R5-C3C down projection + weighted top-k numeric parity, still no full-MoE speed/default claim |
