...
- **Next gate:** P2-O/P3-B/P3-C/P3-D/P3-E Spark artifacts; P3-D banks opt-in server smoke only and P3-E banks RC quality smoke only, while default promotion and full leaderboard quality still require the follow-up release battery. P4B native zero-shadow has banked fail-loud route integrity plus a single-CTA output-returning numeric reference plus two microbench anti-proofs (single-CTA serial is slow; multi-CTA active recompute is slower). **P4C active-reuse bridge/profile/basic smoke are banked, but RC-mini is rejected:** `fused_zero_shadow_active_reuse_contract` passes on the Spark real-runner path; component profile shows gate/up is the **63.8%** main cost; tile2 basic server smoke records P4C call delta **240**, **40** layers called, text-exact **2/2**, release/reload healthy; wider rc-mini is only completion exact **3/6** (chat **2/2**), and shadow-cycle first-divergence only proves the arithmetic prompt keeps top-1 for 8 steps while logits already drift. P4C fused speed/default promotion remain closed; the next cut is hard go/no-go and MTP runtime overhead reduction, not wider P4C RC.
