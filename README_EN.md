...
>
> **✅ Latest banked Lynn-native core win:** 35B NVFP4 serving now releases the decode-phase BF16 dequant-shadow from resident memory: **88→28 GiB resident (~60 GiB freed)**, token-exact, 0.998× TPS (no regression). `server/openai_http.py` wires the `reload→prefill→release→decode` service cycle. **P0.1/P0.2 packed-prefill gates have now passed:** after releasing the 60 GiB shadow, `stream_bf16` proves token-exact no-reload prefill (peak **40.28 GiB**, proof prefill **20.75 s**); P0.2 resident inventory shows only **4.72 GiB** BF16 after release. **2026-06-04 P1 single dense projection PoC passed:** real `linear_attn.in_proj_qkv` runs directly from packed E2M1 + FP16 scale through Triton matvec, with no BF16 shadow, numeric PASS, 32.00→10.00 MiB (3.20×), **160.29us vs BF16 190.03us = 1.186×**. **P1-A naive + tiled scalar batched bridges are both rejected:** numeric/no-shadow passed, but M>1 still loses to BF16 tensor-core GEMM. **P2 grouped MoE has reached P2-D:** active packed MoE drops the `stream_bf16` proof from **0.49-0.51 s/layer** to **23.83ms/layer**; P2-D adds router/shared back and still passes numeric/no-shadow, M64 **29.21ms/layer**, ~**17x faster than stream_bf16**, but only **0.741x vs BF16 full MoE**. Next work is cutting router/grouping + packed-active scheduling cost, not wiring it into serving prematurely. Python is the control/verification plane; catching llama.cpp requires native kernels.
...
[阅读中文版](README.md) · [Chinese Zhihu June series: Qwen 3.6 35B-A3B custom inference engine](https://zhuanlan.zhihu.com/p/2045562329396400486) · [Strategy](docs/STRATEGY.md) · [Architecture](docs/DESIGN.md) · **[🆕 6/3 Restart Notes](RELEASE_NOTES_20260603.md)** · [P1 dense projection PoC](reports/stage6/P1_DENSE_PROJECTION_POC_20260604.md) · [P1-A tiled sweep](reports/stage6/P1A_TILED_PROJECTION_SWEEP_20260604.md) · [P2 grouped MoE census](reports/stage6/P2_GROUPED_MOE_PREFILL_CENSUS_20260604.md) · [P2-D one-layer MoE hybrid](reports/stage6/P2D_ONE_LAYER_MOE_HYBRID_POC_20260604.md) · [5/20 historical notes](RELEASE_NOTES_20260520.md)
...
- **P2-A/B/C/D evidence:** single-expert gate/up loses to BF16; routed gate/up + down active path passed (M64=23.83ms/layer, ~21x faster than `stream_bf16`); P2-D adds router/shared back and still passes numeric/no-shadow, M64=29.21ms/layer, ~17x faster than `stream_bf16`, but 0.741x vs BF16 full MoE.
- **Next gate:** P2-E cuts eager router/grouping and packed-active scheduling cost; dense/MoE M>1 needs a native FP4-MMA/CUTLASS-style bridge to catch BF16.
