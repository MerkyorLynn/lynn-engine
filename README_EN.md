...
| **Packed prefill** | P0.1/P0.2 passed:no-reload `stream_bf16` is token-exact, peak **40.28 GiB**, proof prefill **20.75s**; after-release BF16 resident is **4.72 GiB**. **P2-O basic passed:** active-MoE no-reload packed-prefill is generated-token exact on 3/3 prompts, 88.16→28.18 GiB, reload calls 0; but opt-in prefill is **149.62s avg (0.008×)**, so this banks correctness/memory only, not speed. |
...
| **B. no-reload prefill** | Extend 28 GiB serving to multi-request without per-request reload | P2-O basic PASS; rc-mini needs slow-mode acceleration or prompt splitting before rerun |
...
- [P2-O basic packed-prefill smoke](reports/stage6/P2O_PACKED_PREFILL_RC_SMOKE_BASIC_20260604.md) · [P2-O runbook](reports/stage6/P2O_PACKED_PREFILL_RC_GATE_RUNBOOK_20260604.md) · [P3/P4 runbook family starts here](reports/stage6/P3_GROUPED_MOE_ZERO_SHADOW_CONTRACT_20260604.md)
...
- **Next gate:** P2-O basic is banked as active-MoE no-reload packed-prefill correctness/memory smoke (3/3 exact, 88.16→28.18 GiB, reload calls 0). rc-mini first failed with invalid `max_seq_len=2048`; the 8192 rerun timed out in slow-mode without a clean result, so it is neither a numeric failure nor a pass. P3-B/P3-C/P3-D/P3-E still wait for Spark artifacts; P3-D banks opt-in server smoke only and P3-E banks RC quality smoke only, while default promotion and full leaderboard quality still require the follow-up release battery. P4B native zero-shadow has banked fail-loud route integrity plus a single-CTA output-returning numeric reference plus two microbench anti-proofs (single-CTA serial is slow; multi-CTA active recompute is slower). **P4C active-reuse bridge/profile/basic smoke are banked, but RC-mini is rejected:** `fused_zero_shadow_active_reuse_contract` passes on the Spark real-runner path; component profile shows gate/up is the **63.8%** main cost; tile2 basic server smoke records P4C call delta **240**, **40** layers called, text-exact **2/2**, release/reload healthy; wider rc-mini is only completion exact **3/6** (chat **2/2**), and shadow-cycle first-divergence only proves the arithmetic prompt keeps top-1 for 8 steps while logits already drift. P4C fused speed/default promotion remain closed; the next cuts are P2/P3 slow-mode acceleration, MTP runtime overhead, and the compiled/C++ hot loop, not wider P4C RC.
