...
>
> **✅ Latest banked Lynn-native core win:** 35B NVFP4 serving now releases the decode-phase BF16 dequant-shadow from resident memory: **88→28 GiB resident (~60 GiB freed)**, token-exact, 0.998× TPS (no regression). `server/openai_http.py` wires the `reload→prefill→release→decode` service cycle. **P0.1 packed-prefill / zero-reload smoke has now passed:** after releasing the 60 GiB shadow, the runner does not call reload; `LYNN_PACKED_PREFILL_SLOW_MODE=stream_bf16` streams temporary per-layer BF16 from resident packed NVFP4, stays token-exact, peaks at **40.28 GiB**, and decodes at 42.47 TPS vs 42.97 baseline. Cost: proof prefill is **20.75 s**. Next: P0.2 resident inventory, then P1/P2 real **C++/CUDA/Triton batched/grouped kernels**. Python is the control/verification plane; catching llama.cpp requires native kernels.
...
- **Next gate:** P0.2 projection/shared resident inventory, then P1/P2 C++/CUDA/Triton batched/grouped packed-prefill kernels.
