...
>
> **✅ Latest banked Lynn-native core win:** 35B NVFP4 serving now releases the decode-phase BF16 dequant-shadow from resident memory: **88→28 GiB resident (~60 GiB freed)**, token-exact, 0.998× TPS (no regression). `server/openai_http.py` wires the `reload→prefill→release→decode` service cycle. **P0.1/P0.2 packed-prefill gates have now passed:** after releasing the 60 GiB shadow, `stream_bf16` proves token-exact no-reload prefill (peak **40.28 GiB**, proof prefill **20.75 s**); P0.2 resident inventory shows only **4.72 GiB** BF16 after release. **P1 dense PoC passed; P1-A batched bridges rejected. P2 grouped MoE has reached P2-KB:** P2-F wires `LYNN_PACKED_PREFILL_SLOW_MODE=p2e_hybrid`; P2-H full `_prefill_layer` mixed L0-3 T16 **45.97ms vs BF16 58.57ms = 1.274x**; P2-I mixed L0-7 T16 **88.96ms vs BF16 113.82ms = 1.279x**, **46.70x** faster than `stream_bf16`, numeric pass, peak **5.123GiB vs stream 17.102GiB**; P2-J exact trace identifies the linear-attn prefill wall: `chunk_gated_delta_with_state` is **71-76%** of traced wall time across T16..512; P2-KA rejects token-by-token reuse of the decode recurrent kernel; **P2-KB block Triton kernel passed the core gate:** T512 **1.16ms vs host loop 16.28ms = 14.04x**, vs chunk **4.55ms = 3.92x**, numeric pass (min cosine **0.999989555**). Next gate: **P2-L opt-in `prefill_linear_attn` integration**. Python is the control/verification plane; catching llama.cpp requires CUDA C++/CUTLASS/native kernels.
...
- **P2-A..P2-KB evidence:** single-expert gate/up loses to BF16; routed active path reaches M64=23.83ms/layer; P2-F engine opt-in mode M64=20.23ms/layer; P2-H/P2-I selected full prefill keeps numeric/no-shadow/speed passing; P2-J trace shows `chunk_gated_delta_with_state` is 71-76% of linear-attn prefill traced wall; P2-KA rejects token-by-token reuse of the decode recurrent kernel; P2-KB one-launch block recurrent kernel passes the core gate (T512 1.16ms, 14.04x vs host loop).
- **Next gate:** P2-L opt-in `prefill_linear_attn` integration; dense/MoE M>1 needs a native FP4-MMA/CUTLASS-style bridge to catch and surpass BF16.
