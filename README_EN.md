...
| **R6000 / FP4-MMA** | Census is banked:**RTX PRO 6000 96GB / 880GiB data workspace** PASS, CUDA capability `[12,0]`, vLLM source candidates **200**, and P76/P79/P85/P87/P103 native FP4 contracts all pass. **R5-C0→R5-C2C passed:** CUTLASS/CuTe exposes an sm120 `mxf4nvf4` block16 E2M1 + UE4M3 scale ABI; CUTLASS 79d native NVF4+UE4M3 grouped GEMM passes host-reference numeric smoke; MoE-shape census proves the next harness must combine 92-style `MoEProblemShape/tokens_per_expert` semantics with 79d-style SM120 execution; R5-C2 selected-expert gate/up passes both schedules; **R5-C2C captures real CUTLASS D/ref row digests and scatters them through the R5-C2B inverse order into `[T, top_k, N_gateup]`. R5-C3A trace-only positive speed signal: gate/up prefill shape `8x256x1024x2048` FP4-MMA is **0.0206ms / 416 TFLOPS**, same-shape BF16 `bmm` is **0.0435ms / 198 TFLOPS**, about **2.11x**.** ABI/numeric/real-D-row scatter smoke only; R5-C3A still does not bank epilogue kernel, SwiGLU/down, full-MoE speed, decode TPS, or defaults. |
...
| **E. native kernels / FP4-MMA** | Grouped CUDA C++/CUTLASS kernels plus FP4-MMA silicon payoff | R5-C2C real D-row slot scatter smoke is banked; R5-C3A gate/up prefill trace shows about **2.11x** vs BF16 bmm; next is R5-C3B SwiGLU/down/weighted-sum numeric parity, still no full-MoE speed/default claim |
