...
| **Current bottleneck** | P4C profile:gate/up **151.67us (63.8%)**, down **86.21us (36.2%)**; gate/up shape sweep found `tile_inter=2,threads=128`: **91.41us = 1.659×**. The next cut wires that shape as a P4C speed candidate. |

**Honest boundary:** Spark sm_121 currently delivers **~44-45 TPS** decode; P4C only banks route/numeric/baseline/diagnostic plus actionable shape-sweep evidence, **not fused speed/default promotion**. Python stays as the control/verification plane; catching llama.cpp requires native kernels, and the full payoff lands on FP4-MMA silicon.
...
- **Next gate:** P2-O/P3-B/P3-C/P3-D/P3-E Spark artifacts; P3-D banks opt-in server smoke only and P3-E banks RC quality smoke only, while default promotion and full leaderboard quality still require the follow-up release battery. P4B native zero-shadow has banked fail-loud route integrity plus a single-CTA output-returning numeric reference plus two microbench anti-proofs (single-CTA serial is slow; multi-CTA active recompute is slower). **The P4C active-reuse runtime bridge plus speed baseline plus component profile plus gate/up shape sweep are banked:** `fused_zero_shadow_active_reuse_contract` passes on the Spark real-runner path; the speed baseline records **1.00013x** vs P4A with exact output/scratch; component profile shows gate/up is the **63.8%** main cost and down is **36.2%**; shape sweep found `tile_inter=2,threads=128` at **91.41us = 1.659x** as an actionable diagnostic. Fused speed/default promotion remain closed; the next cut is wiring that gate/up shape as a P4C speed candidate, then rerunning P4C baseline/RC. Dense/MoE M>1 needs a native FP4-MMA/CUTLASS-style bridge to catch and surpass BF16.
