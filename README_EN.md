...
- **Next gate:** P2-O/P3-B/P3-C/P3-D/P3-E Spark artifacts; P3-D banks opt-in server smoke only and P3-E banks RC quality smoke only, while default promotion and full leaderboard quality still require the follow-up release battery. P4B native zero-shadow has banked fail-loud route integrity plus a single-CTA output-returning numeric reference plus a microbench anti-proof; the next implementation must be multi-CTA/CUTLASS-style, while fused-kernel speed/default promotion remain closed. Dense/MoE M>1 needs a native FP4-MMA/CUTLASS-style bridge to catch and surpass BF16.
