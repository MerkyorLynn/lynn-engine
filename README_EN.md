...
| **Native zero-shadow MoE** | P4B numeric reference is exact, but single-CTA and active recompute were rejected by microbench; P4C active-reuse runtime bridge + speed baseline + component profile are banked. |
...
| **Runtime opt-in** | `LYNN_NATIVE_GATEUP_TILE_INTER=2` passed real resident-runner preflight: native call delta=1, recorded tile=2, candidate vs Triton `rel_l2=0/max_abs=0`. |

**Honest boundary:** Spark sm_121 currently delivers **~44-45 TPS** decode; P4C only banks route/numeric/baseline/diagnostic plus an opt-in shape candidate, **not fused speed/default promotion**. The next gate reruns server/RC with `LYNN_NATIVE_GATEUP_TILE_INTER=2`. Python stays as the control/verification plane; catching llama.cpp requires native kernels, and the full payoff lands on FP4-MMA silicon.
