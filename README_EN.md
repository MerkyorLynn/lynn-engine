...
- **Next gate:** P2-O/P3-B/P3-C/P3-D/P3-E Spark artifacts; P3-D banks opt-in server smoke only and P3-E banks RC quality smoke only, while default promotion and full leaderboard quality still require the follow-up release battery. P4B native zero-shadow has banked fail-loud route integrity plus a single-CTA output-returning numeric reference plus two microbench anti-proofs (single-CTA serial is slow; multi-CTA active recompute is slower). **The P4C active-reuse backend is wired:** the next `fused_zero_shadow_active_reuse_contract` implementation must compute and reuse `active[top_k,512]` once, prefer a two-phase/CUTLASS-style shape, and must not relabel active recompute as fused speed; it is waiting for a Spark runtime bridge artifact, and fused-kernel speed/default promotion remain closed. Dense/MoE M>1 needs a native FP4-MMA/CUTLASS-style bridge to catch and surpass BF16.
