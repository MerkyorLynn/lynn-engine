...
| **R6000 / FP4-MMA** | Census is banked:**RTX PRO 6000 96GB / 880GiB data workspace** PASS, CUDA capability `[12,0]`, vLLM source candidates **200**, and P76/P79/P85/P87/P103 native FP4 contracts all pass. **R5-C0/R5-C1/R5-C2A/R5-C2 passed:** CUTLASS/CuTe exposes an sm120 `mxf4nvf4` block16 E2M1 + UE4M3 scale ABI; CUTLASS 79d native NVF4+UE4M3 grouped GEMM passes host-reference numeric smoke; MoE-shape census proves the next harness must combine 92-style `MoEProblemShape/tokens_per_expert` semantics with 79d-style SM120 execution; R5-C2 maps selected-expert `tokens_per_expert=[32,64,64,96]` to per-group M shapes and passes host-reference on both schedules. ABI/numeric-smoke/source-census/selected-expert gate-up smoke only, no grouped-MoE speed/default bank. |
...
| **E. native kernels / FP4-MMA** | Grouped CUDA C++/CUTLASS kernels plus FP4-MMA silicon payoff | R5-C2 selected-expert gate/up numeric smoke is banked; next is R5-C2B slot-preserving selected-output bridge, still no grouped-MoE speed/default claim |
...
Cooperative/Pingpong schedules pass host-reference verification. The next gate
is R5-C2B slot-preserving selected-output bridge, not a grouped-MoE FP4-MMA kernel or
default speed promotion claim.
