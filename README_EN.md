...
**Lynn engine is restarted as a parallel mainline:** the client can still use llama.cpp/GGUF as the pragmatic short-term backend, but the engine is again chasing same-model, same-hardware parity against llama.cpp. The endgame is not “switch frameworks”; it is owning Lynn's **serving memory lifecycle, MTP runtime, C++/compiled decode hot loop, CUDA C++/CUTLASS/native kernels**. The 4-bit / zero-shadow work is a memory and cross-device kernel asset, not a single Spark 45→70 TPS speed lever.

| Area | Banked evidence |
|---|---|
| **Serving memory** | 35B NVFP4 decode releases the BF16 dequant-shadow:**88→28 GiB resident (~60 GiB freed)**, token-exact, **0.998×** TPS (no regression). `server/openai_http.py` wires `reload→prefill→release→decode`. |
| **Packed prefill** | P0.1/P0.2 passed:no-reload `stream_bf16` is token-exact, peak **40.28 GiB**, proof prefill **20.75s**; after-release BF16 resident is **4.72 GiB**. **P2-O basic passed:** 3/3 exact, 88.16→28.18 GiB, reload calls 0. **P2-O rc-mini non-long shard passed:** 5/5 exact, same 60 GiB release, reload calls 0; but opt-in prefill is 108-150s avg (0.008×), so this banks correctness/memory only, not speed. |
| **Dense / prefill path** | P1 dense PoC passed; P1-A batched bridges rejected; P2 grouped MoE reached P2-N: layers 0-7 + block linear-attn, T16 **1.541×**, T64 **1.140×** vs BF16, numeric/no-shadow pass. |
| **P3 contract** | P3-A layer0 grouped active-MoE contract probe passed: numeric true, shadow absent at candidate start, argmax **3/3**, packed active bytes **0.563 GiB** vs BF16 active **1.500 GiB**, but speed is **0.744×** vs BF16 active; batched-down diagnostic also passes numeric and nudges to **0.760×**, still below the speed gate. |
| **P3 selected prefill** | P3-B layers 0-3 selected stack passed: cosine min **0.999949**, argmax true, active BF16 shadow absent, reload not called, active-shadow drop **5.991 GiB**, avg **1.020×** vs P2-N reference; selected-layer composition only, no server/RC/default bank. |
| **P3 resident prompt** | P3-C basic passed: 3/3 generated-ID exact, 88.161→28.178 GiB, **60.000 GiB** released, reload not called; candidate prefill avg **151.861s** (0.008×), so this banks real-prompt correctness/memory only, not server/RC/default/throughput. |
...
| **R6000 / FP4-MMA** | Census is banked:**RTX PRO 6000 96GB / 880GiB data workspace** PASS, CUDA capability `[12,0]`, vLLM source candidates **200**, and P76/P79/P85/P87/P103 native FP4 contracts all pass. **R5-C0→R5-C3C passed:** R5-C2C captures real CUTLASS D/ref row digests and scatters them through the R5-C2B inverse order into `[T, top_k, N_gateup]`; R5-C3A trace-only positive speed signal: gate/up prefill shape `8x256x1024x2048` FP4-MMA is **0.0206ms / 416 TFLOPS**, same-shape BF16 `bmm` is **0.0435ms / 198 TFLOPS**, about **2.11x**; R5-C3B captures full D/ref row values plus exact bits, scatter max_abs=0, and matching host-SwiGLU checksums on both schedules; **R5-C3C composes those real gate/up values through host SwiGLU + deterministic down + weighted top-k, with max_abs 0.0 on both schedules.** ABI/numeric/composition parity only; still no full-MoE speed, decode TPS, or defaults. |
...
| Track | Target | Current gate |
|---|---|---|
| **A. 60 GiB serving** | Use 28 GiB resident for single long sessions / long decode | Shipped; multi-request release/reload still pays ~23s reload |
...
| **E. native kernels / FP4-MMA** | Grouped CUDA C++/CUTLASS kernels plus FP4-MMA silicon payoff | R5-C3C down + weighted top-k parity is banked; next is full active-MoE prefill speed A/B, still no decode TPS/default claim |
...
- [中文 README](README.md) · [Chinese Zhihu June series](https://zhuanlan.zhihu.com/p/2045562329396400486) · [Strategy](docs/STRATEGY.md) · [Architecture](docs/DESIGN.md)
...
- [P4C active-reuse decision](reports/stage6/P4C_ACTIVE_REUSE_KERNEL_DECISION_20260604.md) · [P4C tile2 basic server smoke](reports/stage6/p4c_tile2_server_smoke_20260604_122443/summary.md) · [Decode GPU-idle ROI probe](reports/stage6/decode_gpu_idle_probe_20260604_154648/summary.md) · [R6000 FP4-MMA PASS census](reports/stage6/r6000_fp4_mma_census_20260604_164457/summary.md) · [R5-A layout bridge PASS](reports/stage6/r5a_layout_bridge_20260604_172706/summary.md) · [R5-B e8m0 repack negative](reports/stage6/r5b_e8m0_repack_20260604_173435/summary.md) · [R5-C CUTLASS UE4M3 ABI PASS](reports/stage6/r5c_cutlass_ue4m3_census_20260604_175216/summary.md) · [R5-C1 CUTLASS numeric smoke PASS](reports/stage6/r5c1_cutlass_numeric_smoke_20260604_181947/summary.md) · [R5-C2A MoE-shape census PASS](reports/stage6/r5c2_moe_shape_census_20260604_183226/summary.md) · [R5-C2 selected-expert gate/up PASS](reports/stage6/r5c2_selected_expert_gateup_smoke_20260604_192904/summary.md) · [R5-C2C real D-row scatter PASS](reports/stage6/r5c2c_real_d_row_slot_scatter_smoke_20260604_201440/summary.md) · [R5-C3A gate/up timing trace](reports/stage6/R5C3A_GATEUP_PREFILL_TIMING_TRACE_20260604.md) · [R5-C3B gate/up value materialization PASS](reports/stage6/r5c3b_gateup_value_materialization_smoke_20260604_204920/summary.md) · [R5-C3C down/weighted parity PASS](reports/stage6/r5c3c_down_weighted_parity_smoke_20260604_130243/summary.md) · [R5-C4 full active-MoE speed A/B contract](reports/stage6/R5C4_FULL_ACTIVE_MOE_PREFILL_SPEED_AB_CONTRACT_20260604.md) · [R6000 grouped-MoE FP4-MMA POC contract](reports/stage6/R6000_GROUPED_MOE_FP4_MMA_POC_CONTRACT_20260604.md)
...
**6/3 strategic correction (supersedes the 5/20 pivot)**: Lynn engine is **restarted as a parallel mainline, target = benchmark against (rival) llama.cpp** — no longer "downgraded to R&D / client defects to llama.cpp". The client still uses llama.cpp/GGUF short-term as a **pragmatic default backend**, but the engine advances in lockstep: same model + hardware vs llama.cpp. The 6/3 measured read (Spark sm_121 has no FP4 MMA → decode structurally capped ~45; parity needs a compiled/C++ hot loop that reduces per-launch host dispatch + native kernels + FP4-MMA silicon, not reusable graph) is in the top 6/3 banner and [6/3 Restart Notes](RELEASE_NOTES_20260603.md); the 5/20 decision is historical background only.
...
- **P2-A..P2-N evidence:** single-expert gate/up loses to BF16; routed active path reaches M64=23.83ms/layer; P2-F engine opt-in mode M64=20.23ms/layer; P2-H/P2-I selected full prefill keeps numeric/no-shadow/speed passing; P2-J trace shows `chunk_gated_delta_with_state` is 71-76% of linear-attn prefill traced wall; P2-KA rejects token-by-token reuse of the decode recurrent kernel; P2-KB/P2-L wire block linear-attn; P2-M/P2-N selected full prefill keeps passing.
- **Next gate:** P2-O basic is banked as active-MoE no-reload packed-prefill correctness/memory smoke (3/3 exact, 88.16→28.18 GiB, reload calls 0). P2-O rc-mini non-long shard (`idx0-4`) is also banked (5/5 exact, same 60 GiB release, reload calls 0). Full rc-mini long-context is still not clean: the 2048 run was config-invalid, and the 8192 full run timed out in slow-mode. P3-A layer0 grouped active-MoE contract probe is banked (numeric/argmax/shadow-absent PASS, but speed 0.744×, no fused-kernel bank). P3-B layers 0-3 selected-prefill is banked (cosine min 0.999949, argmax true, reload not called, avg 1.020× vs P2-N, but no server/RC/default bank). P3-C basic resident prompt is also banked (3/3 generated-ID exact, 88.161→28.178 GiB, reload not called, but prefill avg is 151.861s, so no throughput/server/default bank). P3-D/P3-E still wait for Spark artifacts; P3-D banks opt-in server smoke only and P3-E banks RC quality smoke only, while default promotion and full leaderboard quality still require the follow-up release battery. P4B native zero-shadow has banked fail-loud route integrity plus a single-CTA output-returning numeric reference plus two microbench anti-proofs (single-CTA serial is slow; multi-CTA active recompute is slower). **P4C active-reuse bridge/profile/basic smoke are banked, but RC-mini is rejected:** `fused_zero_shadow_active_reuse_contract` passes on the Spark real-runner path; component profile shows gate/up is the **63.8%** main cost; tile2 basic server smoke records P4C call delta **240**, **40** layers called, text-exact **2/2**, release/reload healthy; wider rc-mini is only completion exact **3/6** (chat **2/2**), and shadow-cycle first-divergence only proves the arithmetic prompt keeps top-1 for 8 steps while logits already drift. P4C fused speed/default promotion remain closed; the next cuts are P3-D server smoke or first cutting P3-C slow-mode, MTP runtime overhead, and the compiled/C++ hot loop, not wider P4C RC.
...
(`PASS_R5A_LAYOUT_BRIDGE_E8M0_REPACK_REQUIRED`). The follow-up R5-B simple e8m0
repack path is now closed negative on R6000 (best rel_l2~0.166 / cosine~0.986).
**R5-C0 is now banked**:
[CUTLASS UE4M3 ABI PASS](reports/stage6/r5c_cutlass_ue4m3_census_20260604_175216/summary.md)
proves CUTLASS/CuTe exposes an sm120 `mxf4nvf4` block16 E2M1 + UE4M3 scale path.
**R5-C1 is also banked**:
[CUTLASS numeric smoke PASS](reports/stage6/r5c1_cutlass_numeric_smoke_20260604_181947/summary.md)
proves CUTLASS 79d native NVF4+UE4M3 grouped GEMM passes host-reference
verification on both schedules. **R5-C2A is also banked**:
[MoE-shape census PASS](reports/stage6/r5c2_moe_shape_census_20260604_183226/summary.md)
proves 79d has SM120 native NVF4+UE4M3 execution but lacks
`MoEProblemShape/tokens_per_expert`, while 92 has MoE shape semantics but uses
Sm100 schedules. **R5-C2 selected-expert gate/up smoke is now banked**:
[R5-C2 selected-expert gate/up PASS](reports/stage6/r5c2_selected_expert_gateup_smoke_20260604_192904/summary.md)
maps `tokens_per_expert=[32,64,64,96]` to 79d per-group M shapes, and both
Cooperative/Pingpong schedules pass host-reference verification. **R5-C2C real
D-row scatter is also banked**:
[R5-C2C real D-row scatter PASS](reports/stage6/r5c2c_real_d_row_slot_scatter_smoke_20260604_201440/summary.md).
It captures real CUTLASS D/ref row digests and scatters them through the R5-C2B
inverse order into `[T, top_k, N_gateup]`; Cooperative/Pingpong each cover 256
rows. This still does not claim an epilogue selected-output kernel, SwiGLU/down,
grouped-MoE FP4-MMA speed, or default speed promotion.
