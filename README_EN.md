...
- **P2-A..P2-L evidence:** single-expert gate/up loses to BF16; routed active path reaches M64=23.83ms/layer; P2-F engine opt-in mode M64=20.23ms/layer; P2-H/P2-I selected full prefill keeps numeric/no-shadow/speed passing; P2-J trace shows `chunk_gated_delta_with_state` is 71-76% of linear-attn prefill traced wall; P2-KA rejects token-by-token reuse of the decode recurrent kernel; P2-KB core gate passes; P2-L opt-in `prefill_linear_attn` integration is 2.56x at T512.
- **Next gate:** P2-M selected-layer/full-prefill smoke; dense/MoE M>1 needs a native FP4-MMA/CUTLASS-style bridge to catch and surpass BF16.
