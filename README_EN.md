...
| **R6000 / FP4-MMA** | Census is banked:**RTX PRO 6000 96GB / 880GiB data workspace** PASS, CUDA capability `[12,0]`, vLLM source candidates **200**, and P76/P79/P85/P87/P103 native FP4 contracts all pass. **R5-C0/R5-C1/R5-C2A passed:** CUTLASS/CuTe exposes an sm120 `mxf4nvf4` block16 E2M1 + UE4M3 scale ABI; CUTLASS 79d native NVF4+UE4M3 grouped GEMM passes host-reference numeric smoke; MoE-shape census proves the next harness must combine 92-style `MoEProblemShape/tokens_per_expert` semantics with 79d-style SM120 execution. ABI/numeric-smoke/source-census only, no selected-expert kernel/speed/default bank. |
...
| **E. native kernels / FP4-MMA** | Grouped CUDA C++/CUTLASS kernels plus FP4-MMA silicon payoff | R5-C2A MoE-shape census is banked; next is a new minimal R5-C2 selected expert gate/up harness, still no grouped-MoE speed/default claim |
...
Sm100 schedules. The next gate is a new minimal R5-C2 selected expert gate/up
harness, not a grouped-MoE FP4-MMA kernel or default speed promotion claim.
