...
| **P3 contract** | P3-A layer0 grouped active-MoE contract probe passed: numeric true, shadow absent at candidate start, argmax **3/3**, packed active bytes **0.563 GiB** vs BF16 active **1.500 GiB**, but speed is **0.744×** vs BF16 active; contract only, no fused-kernel bank. |
...
| **D. compiled/C++ hot loop** | Reduce per-launch host/Python dispatch, not CUDA graph | Medium-term runtime work; reusable graph is closed at 0.75× |
| **E. native kernels / FP4-MMA** | Grouped CUDA C++/CUTLASS kernels plus FP4-MMA silicon payoff | Long-term moat; P4C is not RC/default clean and remains diagnostic |
