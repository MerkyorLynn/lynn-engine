...
| P2-KA gated-delta native recurrent loop | **NUMERIC PASS / SPEED FAIL 2026-06-04**: existing single-token Triton recurrent kernel matches chunk reference (min cosine 0.999989555, argmax match), but one launch per token regresses after T128; T512 native loop 15.62ms vs chunk 4.16ms (0.266x) | see `reports/stage6/P2KA_GATED_DELTA_NATIVE_LOOP_POC_20260604.md`; reject loop reuse, continue to P2-KB true chunk/block-level gated-delta prefill kernel |
| P2-KB gated-delta block kernel | **CORE KERNEL PASSED 2026-06-04**: one Triton launch loops over T tokens; numeric pass vs chunk reference (min cosine 0.999989555, argmax match) and fixes P2-KA launch failure; T512 1.16ms vs host loop 16.28ms (14.04x), vs chunk 4.55ms (3.92x) | see `reports/stage6/P2KB_GATED_DELTA_BLOCK_KERNEL_POC_20260604.md`; continue to P2-L opt-in `prefill_linear_attn` integration before server promotion |
| P2-L linear-attn block integration | **PASSED 2026-06-04**: `LYNN_LINEAR_ATTN_PREFILL_BLOCK_GQA=1` wires the block kernel into `prefill_linear_attn`; T16..512 output/state/conv numeric pass; T512 2.18ms vs reference 5.58ms (2.56x) | see `reports/stage6/P2L_LINEAR_ATTN_BLOCK_INTEGRATION_SMOKE_20260604.md`; continue to P2-M selected-layer/full-prefill smoke |
| P2-M selected-layer block-linear smoke | **PASSED 2026-06-04**: selected real layers 0-3 with P2E MoE + block linear-attn numeric/no-shadow/speed pass; T16 39.12ms vs BF16 57.40ms (1.467x), T64 84.26ms vs BF16 93.59ms (1.111x), and faster than P2E-only | see `reports/stage6/P2M_SELECTED_LAYER_BLOCK_LINEAR_SMOKE_20260604.md`; continue to P2-N wider layer coverage and RC/server smoke |
| P2-N wider-layer block-linear smoke | **PASSED 2026-06-04**: layers 0-7 (six linear-attn + two full-attn) with P2E MoE + block linear-attn numeric/no-shadow/speed pass; T16 74.98ms vs BF16 115.56ms (1.541x), T64 162.25ms vs BF16 185.03ms (1.140x), and faster than P2E-only | see `reports/stage6/P2N_WIDER_LAYER_BLOCK_LINEAR_SMOKE_20260604.md`; continue to RC/server smoke before any default promotion |
