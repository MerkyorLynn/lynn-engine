...
|---|---:|
| Greedy exact | 6/6 |
| Mean decode TPS, baseline | 45.04 |
| Mean decode TPS, fused gate/up | 46.36 |
| Mean speedup | 1.0293x |
| Minimum speedup | 1.0206x |
...
9B profile yet.  The direct resident replay sees a clean 2-4% gain, while the
OpenAI service gate is effectively flat at 512 tokens.  This confirms the
boundary is safe and exact, but the current service bottleneck is no longer
only the dense gate/up split.
...
- `reports/qwen35_9b/p169_dense_gateup_resident_gate_20260519_0755_densegateup.json`
