...
Main intake commit tested this candidate through the current p134 routed-only fixture wrapper on R6000 with:

```bash
ROUTED_ONLY=1 \
CANDIDATE_BACKEND=native_output_owned_bf16 \
MAX_ABS_THRESHOLD=0.02 \
COSINE_THRESHOLD=0.99998 \
ALLOW_NONEXACT=1 \
WARMUP=10 \
ITERS=50 \
...
| Metric | Value | Status |
|---|---:|---|
| p134 routed-only pass | 18/18 | GREEN under relaxed numeric threshold |
| exact_count | 0/18 | NOT strict-default safe |
| max_abs_max | 3.90625e-3 | AMBER numeric |
| rel_l2_max | 6.8287e-3 | AMBER numeric |
| cosine_min | 0.999980211 | GREEN for exploratory gate |
| candidate_ms_mean | 0.050471 ms | FAST_CANDIDATE timing |
| p134 PyTorch fixture ref_ms_mean | 0.926681 ms | reference only |

Verdict: this is a real output-owned scheduling win and should drive the NVFP4/W4A16 repack path. It is not a default-promote candidate because it is non-exact and currently uses BF16 dequantized weights, not production packed NVFP4 weights.

Artifacts:

- `reports/qwen36_35b/p134_routed_only_native_output_owned_bf16_20260518_output_owned_bf16.json`
- `reports/qwen36_35b/p134_routed_only_native_output_owned_bf16_20260518_output_owned_bf16.summary.json`
- `reports/qwen36_35b/p134_routed_only_native_output_owned_bf16_20260518_output_owned_bf16.strict.summary.json`

## Original Branch Report
