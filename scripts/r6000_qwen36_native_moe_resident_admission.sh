...
P138_FIXTURE_DIR="${P138_FIXTURE_DIR:-${REPORT_ROOT}/p138_packed_fixtures}"
...
STRUCTURED_PROBE="${REPO_ROOT}/scripts/openai_structured_tps_gate.py"
...
# ─────────────────────────────────────────────────────────────────────────────
MISSING=()
for probe in "$P136_PROBE" "$P139_PROBE" "$P37_PROBE" "$P25_PROBE" "$STRUCTURED_PROBE"; do
  if [[ ! -f "$probe" ]]; then
    MISSING+=("$probe")
  fi
done
...
_write_summary() {
  local decision="$1"
  local reason="$2"
  local p25_tps="${3:-null}"
  mkdir -p "$OUT_DIR"
  "$PY" - "$SUMMARY_JSON" "$CANDIDATE_NAME" "$decision" "$reason" "$P37_EXACT" "$p25_tps" "$STAMP" <<'PYSUMMARY'
...
if [[ "$DRY_RUN" == "1" ]]; then
  ...
  mkdir -p "$OUT_DIR"
  if "$PY" "$P136_PROBE" --fixtures "$FIXTURE_DIR" --model-dir "$MODEL" --out "$P136_OUT"; then
...
if [[ "$DRY_RUN" == "1" ]]; then
  run_or_print "P139" \
    "$PY" "$P139_PROBE" \
      --p138-fixtures "$P138_FIXTURE_DIR" \
      --p135-fixtures "$FIXTURE_DIR" \
      --out "$P139_OUT"
  record_stage "p139_packed" "DRY_RUN"
else
  if "$PY" "$P139_PROBE" --p138-fixtures "$P138_FIXTURE_DIR" --p135-fixtures "$FIXTURE_DIR" --out "$P139_OUT"; then
...
if [[ "$DRY_RUN" == "1" ]]; then
  run_or_print "P37-exact" \
    "$PY" "$P37_PROBE" \
      ...
      --out "$P37_OUT" \
      --max-new "$P37_MAX_NEW" \
      ${P37_CANDIDATE_ARGS[@]+"${P37_CANDIDATE_ARGS[@]}"}
  record_stage "p37_exact" "DRY_RUN"
else
  if "$PY" "$P37_PROBE" --model "$MODEL" --out "$P37_OUT" --max-new "$P37_MAX_NEW" ${P37_CANDIDATE_ARGS[@]+"${P37_CANDIDATE_ARGS[@]}"}; then
...
if [[ "$DRY_RUN" == "1" ]]; then
  ...
  if "$PY" "$P25_PROBE" --url "$BASE_URL" --model "$SERVED_MODEL" --max-tokens $P25_MAX_TOKENS --runs 3 --chat --out "$P25_OUT"; then
...
if [[ "$DRY_RUN" == "1" ]]; then
  ...
  "$PY" "$STRUCTURED_PROBE" \
    --base-url "$SERVER_ROOT" --model "$SERVED_MODEL" \
    --out "$STRUCT_40_OUT" --prompt-specs-file "$STRUCTURED_PROMPTS" --requests "$STRUCTURED_40" --max-tokens 128 --timeout 120 \
    && record_stage "structured_40" "PASS" \
    || record_stage "structured_40" "FAIL"
  "$PY" "$STRUCTURED_PROBE" \
    --base-url "$SERVER_ROOT" --model "$SERVED_MODEL" \
    --out "$STRUCT_70_OUT" --prompt-specs-file "$STRUCTURED_PROMPTS" --requests "$STRUCTURED_70" --max-tokens 128 --timeout 120 \
...
tps = (
    d.get('decode_tps_median')
    or d.get('median_tps')
    or d.get('tps_512')
    or (((d.get('summary_by_max_tokens') or {}).get('512') or {}).get('decode_tps') or {}).get('median')
)
print(tps if tps else 'null')
" 2>/dev/null || echo "null")"
fi

_write_summary "$DECISION" "$REASON" "$P25_512_TPS"
