...
CANDIDATE_OUTPUT_DIR=${CANDIDATE_OUTPUT_DIR:-$REPORT_DIR/p176_recurrent_from_outconv_ab_candidate_${STAMP}}
REPORT=${REPORT:-$REPORT_DIR/p176_recurrent_from_outconv_ab_candidate_${STAMP}.json}
P169_OUT=${P169_OUT:-$REPORT_DIR/p176_recurrent_from_outconv_ab_candidate_${STAMP}_p169_check.json}
DEVICE=${DEVICE:-cuda}
...
"$PY" benchmarks/p176_qwen36_recurrent_from_outconv_ab_candidate.py \
  --model "$MODEL" \
  --fixtures "$FIXTURES" \
...
echo "[p176] candidate-output-dir: $CANDIDATE_OUTPUT_DIR"
echo "[p176] report: $REPORT"
echo "[p176] p169 check report: $P169_OUT"
