...
OUT_JSON="${OUT_JSON:-${REPORT_DIR}/p168_dense_ffn_fused_gateup_probe_${STAMP}.json}"
DEVICE="${DEVICE:-cuda}"
DTYPE="${DTYPE:-bf16}"
WARMUP="${WARMUP:-8}"
REPEAT="${REPEAT:-64}"
...
if [[ -z "$FIXTURE_DIR" ]]; then
  ...
  if [[ -z "$latest_fixture" ]]; then
    echo "[p168] no P159 fixture dir found under $REPORT_DIR; set FIXTURE_DIR=/path/to/p159_dense_ffn_fixtures_*" >&2
    exit 2
  fi
...
echo "[p168] root=$ROOT"
echo "[p168] python=$PYTHON_BIN"
echo "[p168] model=$MODEL"
echo "[p168] fixtures=$FIXTURE_DIR"
echo "[p168] out=$OUT_JSON"

"$PYTHON_BIN" benchmarks/p168_qwen35_9b_dense_gateup_fused_probe.py \
...
echo "[p168] done report=$OUT_JSON"
