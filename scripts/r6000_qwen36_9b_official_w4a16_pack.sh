...
# R6000 CPU/network pipeline for the official Qwen3.6-9B-Dense route:
...
BF16_MODEL="${BF16_MODEL:-$MODEL_ROOT/Qwen3.6-9B-Dense-BF16}"
OUT_MODEL="${OUT_MODEL:-$MODEL_ROOT/Qwen3.6-9B-Dense-lynn-native-w4a16-nvfp4-v0}"
REPORT_ROOT="${REPORT_ROOT:-/root/autodl-tmp/reports}"
REPORT_DIR="${REPORT_DIR:-$REPORT_ROOT/qwen36_9b}"
STAMP="${STAMP:-$(date +%Y%m%d_%H%M%S)}"
LOG="${LOG:-$REPORT_DIR/r6000_qwen36_9b_official_w4a16_pack_${STAMP}.log}"

DOWNLOAD_SOURCE="${DOWNLOAD_SOURCE:-auto}"
MS_REPO="${MS_REPO:-Qwen/Qwen3.6-9B-Dense}"
HF_REPO="${HF_REPO:-Qwen/Qwen3.6-9B-Dense}"
...
validate_model() {
  "$PYTHON_BIN" - "$BF16_MODEL" "$REPORT_DIR/r6000_qwen36_9b_bf16_validate_${STAMP}.json" <<'PY'
from __future__ import annotations
...
if not index_path.exists():
    report = {
        "schema_version": "qwen36-9b-bf16-safetensors-validate-v1",
        "model": str(model),
        "ok": False,
...
summary = {
    "schema_version": "qwen36-9b-official-w4a16-pack-summary-v1",
    "stamp": stamp,
    "bf16_model": str(bf16),
...
path = report_dir / f"r6000_qwen36_9b_official_w4a16_pack_summary_{stamp}.json"
...
{
  echo "[qwen36-9b-pack] start $(date)"
  echo "[qwen36-9b-pack] root=$ROOT"
  echo "[qwen36-9b-pack] bf16=$BF16_MODEL"
  echo "[qwen36-9b-pack] out_model=$OUT_MODEL"
  echo "[qwen36-9b-pack] download_source=$DOWNLOAD_SOURCE ms=$MS_REPO hf=$HF_REPO"
  ...
  if [[ ! -f "$BF16_MODEL/model.safetensors.index.json" ]]; then
    if [[ "$DOWNLOAD_IF_MISSING" != "1" ]]; then
      echo "[qwen36-9b-pack][fail] missing BF16 model index: $BF16_MODEL" >&2
      exit 2
    fi
    echo "[qwen36-9b-pack] downloading official BF16"
    download_model
  else
    echo "[qwen36-9b-pack] BF16 index already present; skip download"
  fi

  echo "[qwen36-9b-pack] validating official safetensors layout"
  validate_model
  ...
  echo "[qwen36-9b-pack] packing BF16 -> Lynn-native W4A16 NVFP4"
  "$PYTHON_BIN" "${pack_args[@]}" \
    > "$REPORT_DIR/r6000_qwen36_9b_w4a16_pack_${STAMP}.json"

  echo "[qwen36-9b-pack] manifest probes"
  "$PYTHON_BIN" benchmarks/quant_manifest_probe.py "$BF16_MODEL" \
    --out "$REPORT_DIR/r6000_qwen36_9b_bf16_manifest_${STAMP}.json"
  "$PYTHON_BIN" benchmarks/quant_manifest_probe.py "$OUT_MODEL" \
    --out "$REPORT_DIR/r6000_qwen36_9b_w4a16_manifest_${STAMP}.json"

  echo "[qwen36-9b-pack] summary"
  write_summary

  echo "[qwen36-9b-pack] done $(date)"
} 2>&1 | tee "$LOG"
