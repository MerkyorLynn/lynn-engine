#!/usr/bin/env python3
"""Same-process A/B: full-attn fusion OFF vs ON (LYNN_FULL_ATTN_FUSED), stacked on the
current best (bh4 + linear-attn flags + fused RMSNorm). Verifies claude-internal's
qk_norm_rope_cache_write + gate_sigmoid_fold kernels: token-coherence + e2e TPS.
"""
...
os.environ.setdefault("LYNN_RMSNORM_FUSED", "1")          # keep the prior win on
os.environ["LYNN_FULL_ATTN_FUSED"] = "0"
...
os.environ["LYNN_FULL_ATTN_FUSED"] = "0"
tpsA, outA = tps()
os.environ["LYNN_FULL_ATTN_FUSED"] = "1"
runner.generate("warm up fused.", max_new=8)
tpsB, outB = tps()

print("=== full-attn fusion: OFF vs ON (on bh4+flags+fused-RMSNorm) ===")
print("A  full-attn-fused OFF : %.3f TPS" % tpsA)
print("B  full-attn-fused ON  : %.3f TPS  (%.3fx vs A)" % (tpsB, tpsB / max(tpsA, 1e-9)))
