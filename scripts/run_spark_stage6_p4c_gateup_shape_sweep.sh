#!/usr/bin/env bash
# Run Stage 6 P4C gate/up launch-shape sweep on Spark and pull artifacts.
set -euo pipefail

usage() {
  cat <<'USAGE'
Usage: run_spark_stage6_p4c_gateup_shape_sweep.sh [options]
...
RUN_NAME="p4c_gateup_shape_sweep_${STAMP}"
REMOTE_RUN_DIR="${REMOTE_REPO}/reports/stage6/${RUN_NAME}"
LOCAL_RUN_DIR="${LOCAL_ROOT}/${RUN_NAME}"

echo "[p4c-gateup-shape-sweep] host=${HOST}"
echo "[p4c-gateup-shape-sweep] expect_head=${EXPECTED_HEAD:-none}"
echo "[p4c-gateup-shape-sweep] remote_run_dir=${REMOTE_RUN_DIR}"
...
docker run --rm --gpus all --ipc=host \
  ...
  -w "$REMOTE_REPO" \
  "$IMAGE" \
  python3 scripts/spark_stage6_p4c_gateup_shape_sweep.py \
...
if [[ -f "$REMOTE_RUN_DIR/result.json" ]]; then
  python3 scripts/summarize_stage6_p4c_gateup_shape_sweep.py \
...
if [[ "$RSYNC_STATUS" -ne 0 ]]; then
  echo "[p4c-gateup-shape-sweep] rsync failed with status ${RSYNC_STATUS}" >&2
  exit "$RSYNC_STATUS"
fi

if [[ ! -f "$LOCAL_RUN_DIR/result.json" ]]; then
  echo "[p4c-gateup-shape-sweep] missing result.json; pulled failure artifacts to ${LOCAL_RUN_DIR}" >&2
...
set +e
python3 scripts/summarize_stage6_p4c_gateup_shape_sweep.py "${SUMMARY_ARGS[@]}"
SUMMARY_STATUS=$?
set -e

echo "[p4c-gateup-shape-sweep] artifacts=${LOCAL_RUN_DIR}"
