#!/usr/bin/env bash
# Run Stage 6 decode GPU-idle probe on Spark and pull artifacts.
set -euo pipefail

usage() {
  cat <<'USAGE'
Usage: run_spark_stage6_decode_gpu_idle_probe.sh [options]
...
Options:
  ...
  --max-new N                  Profile N and 2N decode tokens. Default: 48.
  --warmup-new N               Warmup decode tokens. Default: 8.
  --allow-provenance-mismatch  Do not fail when both HEAD and manifest differ.
  --no-strict                  Pull artifacts but do not fail on summary verdict.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --warmup-new) WARMUP_NEW="$2"; shift 2 ;;
...
RUN_NAME="decode_gpu_idle_probe_${STAMP}"
REMOTE_RUN_DIR="${REMOTE_REPO}/reports/stage6/${RUN_NAME}"
LOCAL_RUN_DIR="${LOCAL_ROOT}/${RUN_NAME}"

echo "[decode-gpu-idle] host=${HOST}"
echo "[decode-gpu-idle] max_new=${MAX_NEW}"
echo "[decode-gpu-idle] expect_head=${EXPECTED_HEAD:-none}"
echo "[decode-gpu-idle] remote_run_dir=${REMOTE_RUN_DIR}"

set +e
ssh "$HOST" \
  "REMOTE_REPO=$(shell_quote "$REMOTE_REPO") REMOTE_RUN_DIR=$(shell_quote "$REMOTE_RUN_DIR") IMAGE=$(shell_quote "$IMAGE") MODEL=$(shell_quote "$MODEL") MAX_NEW=$(shell_quote "$MAX_NEW") WARMUP_NEW=$(shell_quote "$WARMUP_NEW") EXPECTED_HEAD=$(shell_quote "$EXPECTED_HEAD") EXPECTED_MANIFEST=$(shell_quote "$EXPECTED_MANIFEST") REQUIRE_PROVENANCE=$(shell_quote "$REQUIRE_PROVENANCE") STRICT=$(shell_quote "$STRICT") bash -s" <<'REMOTE'
set -euo pipefail
file_sha256() {
...
docker run --rm --gpus all --ipc=host \
  ...
  -w "$REMOTE_REPO" \
  "$IMAGE" \
  python3 scripts/spark_stage6_decode_gpu_idle_probe.py \
    --model "$MODEL" \
...
if [[ -f "$REMOTE_RUN_DIR/result.json" ]]; then
  python3 scripts/summarize_stage6_decode_gpu_idle_probe.py \
...
if [[ "$RSYNC_STATUS" -ne 0 ]]; then
  echo "[decode-gpu-idle] rsync failed with status ${RSYNC_STATUS}" >&2
  exit "$RSYNC_STATUS"
fi

if [[ ! -f "$LOCAL_RUN_DIR/result.json" ]]; then
  echo "[decode-gpu-idle] missing result.json; pulled failure artifacts to ${LOCAL_RUN_DIR}" >&2
...
set +e
python3 scripts/summarize_stage6_decode_gpu_idle_probe.py "${SUMMARY_ARGS[@]}"
SUMMARY_STATUS=$?
set -e

echo "[decode-gpu-idle] artifacts=${LOCAL_RUN_DIR}"
