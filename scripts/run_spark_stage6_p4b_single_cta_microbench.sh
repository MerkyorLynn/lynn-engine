#!/usr/bin/env bash
# Run Stage 6 P4B single-CTA microbench on Spark and pull artifacts.
set -euo pipefail

usage() {
  cat <<'USAGE'
Usage: run_spark_stage6_p4b_single_cta_microbench.sh [options]
...
PROVENANCE_FILES=(
  ...
  "scripts/test_stage6_p4b_single_cta_microbench_tools.py"
...
echo "[p4b-single-cta-microbench] host=${HOST}"
echo "[p4b-single-cta-microbench] expect_head=${EXPECTED_HEAD:-none}"
echo "[p4b-single-cta-microbench] remote_run_dir=${REMOTE_RUN_DIR}"
...
docker run --rm --gpus all --ipc=host \
  ...
  -w "$REMOTE_REPO" \
  "$IMAGE" \
  python3 scripts/spark_stage6_p4b_single_cta_microbench.py \
...
if [[ -f "$REMOTE_RUN_DIR/result.json" ]]; then
  python3 scripts/summarize_stage6_p4b_single_cta_microbench.py \
...
if [[ "$RSYNC_STATUS" -ne 0 ]]; then
  echo "[p4b-single-cta-microbench] rsync failed with status ${RSYNC_STATUS}" >&2
  exit "$RSYNC_STATUS"
fi

if [[ ! -f "$LOCAL_RUN_DIR/result.json" ]]; then
  echo "[p4b-single-cta-microbench] missing result.json; pulled failure artifacts to ${LOCAL_RUN_DIR}" >&2
...
set +e
python3 scripts/summarize_stage6_p4b_single_cta_microbench.py "${SUMMARY_ARGS[@]}"
SUMMARY_STATUS=$?
set -e

echo "[p4b-single-cta-microbench] artifacts=${LOCAL_RUN_DIR}"
