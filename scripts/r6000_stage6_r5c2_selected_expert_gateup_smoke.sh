#!/usr/bin/env bash
# R6000 wrapper for Stage 6 R5-C2 selected-expert gate/up numeric smoke.
set -euo pipefail
...
OUT_DIR="${OUT_DIR:-${ROOT}/reports/stage6/r5c2_selected_expert_gateup_smoke_${STAMP}}"
...
set +e
"$PYTHON_BIN" "$ROOT/scripts/r6000_stage6_r5c2_selected_expert_gateup_smoke.py" \
...
summary_rc=0
"$PYTHON_BIN" "$ROOT/scripts/summarize_stage6_r5c2_selected_expert_gateup_smoke.py" \
  "$OUT_DIR/result.json" \
  --markdown-out "$OUT_DIR/summary.md" \
  --strict-exit || summary_rc=$?

echo "[r5c2-selected] artifact=$OUT_DIR"
