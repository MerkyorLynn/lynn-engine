#!/usr/bin/env python3
"""Same-process A/B: eager _rms_norm vs fused Triton RMSNorm (LYNN_RMSNORM_FUSED).

The 2 layer-norms/layer (input + post-attn, 80/token) are torch-eager (~6-8 launches
each). The fused path uses the existing rmsnorm_triton (1 launch) with the cached
(1.0+weight) offset. Toggle is read at runtime, so we A/B in one load/prompt on top of
bh4 + linear-attn flags. Expect ~token-exact (same FP32 math) + faster (launch cut).
"""
...
os.environ["LYNN_RMSNORM_FUSED"] = "0"
...
os.environ["LYNN_RMSNORM_FUSED"] = "0"
tpsA, outA = tps()
os.environ["LYNN_RMSNORM_FUSED"] = "1"
runner.generate("warm up fused.", max_new=8)
tpsB, outB = tps()

print("=== _rms_norm: eager vs fused Triton (bh4 + linear-attn flags) ===")
print("A  eager rmsnorm  : %.3f TPS" % tpsA)
print("B  fused rmsnorm  : %.3f TPS  (%.3fx vs A)" % (tpsB, tpsB / max(tpsA, 1e-9)))
