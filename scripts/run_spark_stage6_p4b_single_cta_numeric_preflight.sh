#!/usr/bin/env bash
# Run Stage 6 P4B single-CTA numeric preflight on Spark and pull artifacts.
set -euo pipefail

usage() {
  cat <<'USAGE'
Usage: run_spark_stage6_p4b_single_cta_numeric_preflight.sh [options]
...
PROVENANCE_FILES=(
  ...
  "scripts/test_stage6_p4b_single_cta_numeric_tools.py"
...
if [[ "$CANDIDATE_MODE" == "multi_cta" ]]; then
  RUN_NAME="p4b_multi_cta_numeric_preflight_${STAMP}"
else
  RUN_NAME="p4b_single_cta_numeric_preflight_${STAMP}"
fi
REMOTE_RUN_DIR="${REMOTE_REPO}/reports/stage6/${RUN_NAME}"
LOCAL_RUN_DIR="${LOCAL_ROOT}/${RUN_NAME}"

echo "[p4b-single-cta] host=${HOST}"
echo "[p4b-single-cta] expect_head=${EXPECTED_HEAD:-none}"
echo "[p4b-single-cta] remote_run_dir=${REMOTE_RUN_DIR}"
...
docker run --rm --gpus all --ipc=host \
  ...
  -w "$REMOTE_REPO" \
  "$IMAGE" \
  python3 scripts/spark_stage6_p4b_single_cta_numeric_preflight.py \
...
if [[ -f "$REMOTE_RUN_DIR/result.json" ]]; then
  python3 scripts/summarize_stage6_p4b_single_cta_numeric_preflight.py \
...
if [[ "$RSYNC_STATUS" -ne 0 ]]; then
  echo "[p4b-single-cta] rsync failed with status ${RSYNC_STATUS}" >&2
  exit "$RSYNC_STATUS"
fi

if [[ ! -f "$LOCAL_RUN_DIR/result.json" ]]; then
  echo "[p4b-single-cta] missing result.json; pulled failure artifacts to ${LOCAL_RUN_DIR}" >&2
...
set +e
python3 scripts/summarize_stage6_p4b_single_cta_numeric_preflight.py "${SUMMARY_ARGS[@]}"
SUMMARY_STATUS=$?
set -e

echo "[p4b-single-cta] artifacts=${LOCAL_RUN_DIR}"
