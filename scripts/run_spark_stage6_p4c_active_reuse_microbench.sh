#!/usr/bin/env bash
# Run Stage 6 P4C active-reuse speed-baseline microbench on Spark and pull artifacts.
set -euo pipefail

usage() {
  cat <<'USAGE'
Usage: run_spark_stage6_p4c_active_reuse_microbench.sh [options]
...
Options:
  ...
  --speed-ratio-floor N        Minimum P4C/P4A ratio for baseline sanity. Default: 0.8.
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --speed-ratio-floor) SPEED_RATIO_FLOOR="$2"; shift 2 ;;
...
RUN_NAME="p4c_active_reuse_microbench_${STAMP}"
REMOTE_RUN_DIR="${REMOTE_REPO}/reports/stage6/${RUN_NAME}"
LOCAL_RUN_DIR="${LOCAL_ROOT}/${RUN_NAME}"

echo "[p4c-active-reuse-microbench] host=${HOST}"
echo "[p4c-active-reuse-microbench] expect_head=${EXPECTED_HEAD:-none}"
echo "[p4c-active-reuse-microbench] remote_run_dir=${REMOTE_RUN_DIR}"

set +e
ssh "$HOST" \
  "REMOTE_REPO=$(shell_quote "$REMOTE_REPO") REMOTE_RUN_DIR=$(shell_quote "$REMOTE_RUN_DIR") IMAGE=$(shell_quote "$IMAGE") WARMUP=$(shell_quote "$WARMUP") ITERS=$(shell_quote "$ITERS") REPEATS=$(shell_quote "$REPEATS") SPEED_RATIO_FLOOR=$(shell_quote "$SPEED_RATIO_FLOOR") EXPECTED_HEAD=$(shell_quote "$EXPECTED_HEAD") EXPECTED_MANIFEST=$(shell_quote "$EXPECTED_MANIFEST") REQUIRE_PROVENANCE=$(shell_quote "$REQUIRE_PROVENANCE") STRICT=$(shell_quote "$STRICT") bash -s" <<'REMOTE'
set -euo pipefail
file_sha256() {
...
docker run --rm --gpus all --ipc=host \
  ...
  -w "$REMOTE_REPO" \
  "$IMAGE" \
  python3 scripts/spark_stage6_p4c_active_reuse_microbench.py \
    --out "$REMOTE_RUN_DIR/result.json" \
    --warmup "$WARMUP" \
    --iters "$ITERS" \
    --repeats "$REPEATS" \
    --speed-ratio-floor "$SPEED_RATIO_FLOOR" \
    $STRICT_FLAG \
...
if [[ -f "$REMOTE_RUN_DIR/result.json" ]]; then
  python3 scripts/summarize_stage6_p4c_active_reuse_microbench.py \
...
if [[ "$RSYNC_STATUS" -ne 0 ]]; then
  echo "[p4c-active-reuse-microbench] rsync failed with status ${RSYNC_STATUS}" >&2
  exit "$RSYNC_STATUS"
fi

if [[ ! -f "$LOCAL_RUN_DIR/result.json" ]]; then
  echo "[p4c-active-reuse-microbench] missing result.json; pulled failure artifacts to ${LOCAL_RUN_DIR}" >&2
...
set +e
python3 scripts/summarize_stage6_p4c_active_reuse_microbench.py "${SUMMARY_ARGS[@]}"
SUMMARY_STATUS=$?
set -e

echo "[p4c-active-reuse-microbench] artifacts=${LOCAL_RUN_DIR}"
