#!/usr/bin/env bash
# R6000 wrapper for Stage 6 R5-C2C real CUTLASS D-row slot scatter smoke.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
CUTLASS_DIR="${CUTLASS_DIR:-/root/autodl-tmp/src/cutlass}"
BUILD_DIR="${BUILD_DIR:-/root/autodl-tmp/src/cutlass/build-r5c2c-sm120a-tools-on}"
PYTHON_BIN="${PYTHON_BIN:-/root/miniconda3/bin/python}"
STAMP="$(date +%Y%m%d_%H%M%S)"
OUT_DIR="${OUT_DIR:-${ROOT}/reports/stage6/r5c2c_real_d_row_slot_scatter_smoke_${STAMP}}"
...
set +e
"$PYTHON_BIN" "$ROOT/scripts/r6000_stage6_r5c2c_real_d_row_slot_scatter_smoke.py" \
  --build \
...
summary_rc=0
"$PYTHON_BIN" "$ROOT/scripts/summarize_stage6_r5c2c_real_d_row_slot_scatter_smoke.py" \
  "$OUT_DIR/result.json" \
  --markdown-out "$OUT_DIR/summary.md" \
  --strict-exit || summary_rc=$?

echo "[r5c2c-real-d-row] artifact=$OUT_DIR"
