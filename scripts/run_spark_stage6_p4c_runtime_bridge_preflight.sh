#!/usr/bin/env bash
# Run Stage 6 P4C active-reuse runtime bridge preflight on Spark and pull artifacts.
set -euo pipefail

usage() {
  cat <<'USAGE'
Usage: run_spark_stage6_p4c_runtime_bridge_preflight.sh [options]
...
RUN_NAME="p4c_runtime_bridge_preflight_${STAMP}"
REMOTE_RUN_DIR="${REMOTE_REPO}/reports/stage6/${RUN_NAME}"
LOCAL_RUN_DIR="${LOCAL_ROOT}/${RUN_NAME}"

echo "[p4c-runtime] host=${HOST}"
echo "[p4c-runtime] expect_head=${EXPECTED_HEAD:-none}"
echo "[p4c-runtime] remote_run_dir=${REMOTE_RUN_DIR}"
...
docker run --rm --gpus all --ipc=host \
  ...
  -w "$REMOTE_REPO" \
  "$IMAGE" \
  python3 scripts/spark_stage6_p4c_runtime_bridge_preflight.py \
    --model "$MODEL" \
...
if [[ -f "$REMOTE_RUN_DIR/result.json" ]]; then
  python3 scripts/summarize_stage6_p4c_runtime_bridge_preflight.py \
...
if [[ "$RSYNC_STATUS" -ne 0 ]]; then
  echo "[p4c-runtime] rsync failed with status ${RSYNC_STATUS}" >&2
  exit "$RSYNC_STATUS"
fi

if [[ ! -f "$LOCAL_RUN_DIR/result.json" ]]; then
  echo "[p4c-runtime] missing result.json; pulled failure artifacts to ${LOCAL_RUN_DIR}" >&2
...
echo "[p4c-runtime] artifacts: ${LOCAL_RUN_DIR}"
