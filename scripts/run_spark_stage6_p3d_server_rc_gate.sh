#!/usr/bin/env bash
# Run Stage 6 P3-D OpenAI server smoke gate on Spark and pull artifacts.
set -euo pipefail

usage() {
  cat <<'USAGE'
Usage: run_spark_stage6_p3d_server_rc_gate.sh [options]
...
Options:
  ...
  --port PORT                  Server port inside the container. Default: 18361.
  ...
  --p3c-pass                   Assert P3-C resident-prompt predecessor passed.
...
RUN_NAME="p3d_${PRESET}_server_rc_gate_${STAMP}"
REMOTE_RUN_DIR="${REMOTE_REPO}/reports/stage6/${RUN_NAME}"
LOCAL_RUN_DIR="${LOCAL_ROOT}/${RUN_NAME}"

echo "[p3d] host=${HOST}"
echo "[p3d] preset=${PRESET}"
echo "[p3d] p3c_pass=${P3C_PASS}"
echo "[p3d] expect_head=${EXPECTED_HEAD:-none}"
echo "[p3d] remote_run_dir=${REMOTE_RUN_DIR}"

set +e
ssh "$HOST" \
  "REMOTE_REPO=$(shell_quote "$REMOTE_REPO") REMOTE_RUN_DIR=$(shell_quote "$REMOTE_RUN_DIR") IMAGE=$(shell_quote "$IMAGE") MODEL=$(shell_quote "$MODEL") PRESET=$(shell_quote "$PRESET") MAX_NEW=$(shell_quote "$MAX_NEW") MAX_SEQ_LEN=$(shell_quote "$MAX_SEQ_LEN") PORT=$(shell_quote "$PORT") EXPECTED_HEAD=$(shell_quote "$EXPECTED_HEAD") EXPECTED_MANIFEST=$(shell_quote "$EXPECTED_MANIFEST") REQUIRE_PROVENANCE=$(shell_quote "$REQUIRE_PROVENANCE") P3C_PASS=$(shell_quote "$P3C_PASS") bash -s" <<'REMOTE'
set -euo pipefail
file_sha256() {
...
docker run --rm --gpus all --ipc=host \
  ...
  -w "$REMOTE_REPO" \
  "$IMAGE" \
  python3 scripts/spark_stage6_p3d_server_rc_gate.py \
    --model "$MODEL" \
    --preset "$PRESET" \
    --max-new "$MAX_NEW" \
...
if [[ "$RSYNC_STATUS" -ne 0 ]]; then
  echo "[p3d] rsync failed with status ${RSYNC_STATUS}" >&2
  exit "$RSYNC_STATUS"
fi

if [[ ! -f "$LOCAL_RUN_DIR/result.json" ]]; then
  echo "[p3d] missing result.json; pulled failure artifacts to ${LOCAL_RUN_DIR}" >&2
  {
    echo "# Stage 6 P3-D Server RC Smoke Summary"
    echo
    echo "| Field | Value |"
    echo "|---|---|"
    ...
    echo
    echo "P3-D could not produce a JSON artifact. Do not bank this run."
  } > "$LOCAL_RUN_DIR/summary.md"
  {
    echo "# Stage 6 Phase 3-D — OpenAI server RC smoke gate"
    echo
    echo "Verdict: **FAIL** (missing result.json)."
    echo
    ...
    echo "## Decision"
    echo
    echo "Do not bank P3-D. Re-run after fixing the missing JSON failure."
...
set +e
python3 scripts/summarize_stage6_p3d_server_rc_gate.py "${SUMMARY_ARGS[@]}"
SUMMARY_STATUS=$?
set -e
...
echo "[p3d] artifacts: ${LOCAL_RUN_DIR}"
