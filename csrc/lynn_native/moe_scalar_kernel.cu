...
constexpr int kStrictGateBlockInter = 8;
constexpr int kStrictGateBlockHidden = 256;
constexpr int kStrictDownTileHidden = 8;
...
template <int BLOCK_INTER, int THREADS>
__global__ void gate_up_silu_strict_tile_kernel(
    const __nv_bfloat16* __restrict__ x,
    const int32_t* __restrict__ expert_ids,
    const uint8_t* __restrict__ gate_up_packed,
    const float* __restrict__ gate_up_scale,
    ...
    int64_t out_stride_i) {
  const int slot = blockIdx.x;
  const int inter_start = blockIdx.y * BLOCK_INTER;
  const int tid = threadIdx.x;
  const int expert = expert_ids[slot];
  const float inv_global = 1.0f / gate_up_global_scale[0];

  float gate_acc[BLOCK_INTER];
  float up_acc[BLOCK_INTER];
#pragma unroll
  for (int r = 0; r < BLOCK_INTER; ++r) {
    gate_acc[r] = 0.0f;
    up_acc[r] = 0.0f;
  }
...
#pragma unroll
  ...
  extern __shared__ float smem[];
  float* gate_s = smem;
  float* up_s = smem + BLOCK_INTER * THREADS;
#pragma unroll
  for (int r = 0; r < BLOCK_INTER; ++r) {
    gate_s[r * THREADS + tid] = gate_acc[r];
    up_s[r * THREADS + tid] = up_acc[r];
...
template <int TILE_HIDDEN, int THREADS>
__global__ void down_weighted_sum_strict_tile_kernel(
    const __nv_bfloat16* __restrict__ inter,
    const int32_t* __restrict__ expert_ids,
    const float* __restrict__ routing_weights,
    const uint8_t* __restrict__ down_packed,
  ...
  float acc[TILE_HIDDEN];
#pragma unroll
  for (int r = 0; r < TILE_HIDDEN; ++r) {
    acc[r] = 0.0f;
  }
  ...
  for (int slot = 0; slot < top_k; ++slot) {
    const int expert = expert_ids[slot];
    const float route = routing_weights[slot];
    for (int i = tx; i < kIntermediate; i += THREADS) {
      const int packed_col = i >> 1;
      const int scale_col = i >> 4;
      const bool low = (i & 1) == 0;
      const float inter_i = __bfloat162float(
      ...
      for (int r = 0; r < TILE_HIDDEN; ++r) {
        ...
        if (hidden < kHidden) {
          ...
          const float scale = down_scale[
              ...
              static_cast<int64_t>(hidden) * scale_stride_m +
              static_cast<int64_t>(scale_col) * scale_stride_g];
          acc[r] += route * e2m1_from_nibble(nibble) * scale * inv_global * inter_i;
        }
      }
    }
  }
...
#pragma unroll
      for (int r = 0; r < TILE_HIDDEN; ++r) {
        row_s[r * THREADS + tx] += row_s[r * THREADS + tx + stride];
      }
    }
    __syncthreads();
  }
...
#pragma unroll
    for (int r = 0; r < TILE_HIDDEN; ++r) {
      const int hidden = tile_start + r;
      if (hidden < kHidden) {
        out[hidden] = __float2bfloat16(row_s[r * THREADS]);
      }
    }
  }
}
...
static torch::Tensor lynn_native_gate_up_silu_strict_tile_impl(
    torch::Tensor x,
    torch::Tensor expert_ids,
    torch::Tensor gate_up_packed,
    torch::Tensor gate_up_scale,
  ...
  const dim3 grid(
      ...
      static_cast<unsigned int>((kIntermediate + kStrictGateBlockInter - 1) / kStrictGateBlockInter));
  const size_t shared_bytes = static_cast<size_t>(2 * kStrictGateBlockInter * kThreads) * sizeof(float);
  gate_up_silu_strict_tile_kernel<kStrictGateBlockInter, kThreads><<<grid, kThreads, shared_bytes>>>(
      reinterpret_cast<const __nv_bfloat16*>(xc.data_ptr<at::BFloat16>()),
      expert_ids_c.data_ptr<int32_t>(),
      packed_c.data_ptr<uint8_t>(),
      scale_c.data_ptr<float>(),
      ...
      out.stride(1));
  const cudaError_t err = cudaGetLastError();
  TORCH_CHECK(err == cudaSuccess, "gate_up_silu_strict_tile_kernel launch failed: ", cudaGetErrorString(err));
  return out;
}

static torch::Tensor lynn_native_down_weighted_sum_strict_tile_impl(
    torch::Tensor inter,
    torch::Tensor expert_ids,
    torch::Tensor routing_weights,
    torch::Tensor down_packed,
  ...
  const int64_t top_k = expert_ids_c.numel();
  const unsigned int grid_x = static_cast<unsigned int>((kHidden + kStrictDownTileHidden - 1) / kStrictDownTileHidden);
  const size_t shared_bytes = static_cast<size_t>(kStrictDownTileHidden * kThreads) * sizeof(float);
  down_weighted_sum_strict_tile_kernel<kStrictDownTileHidden, kThreads><<<grid_x, kThreads, shared_bytes>>>(
      reinterpret_cast<const __nv_bfloat16*>(inter_c.data_ptr<at::BFloat16>()),
      expert_ids_c.data_ptr<int32_t>(),
      routing_weights_c.data_ptr<float>(),
      packed_c.data_ptr<uint8_t>(),
      ...
      top_k);
  const cudaError_t err = cudaGetLastError();
  TORCH_CHECK(err == cudaSuccess, "down_weighted_sum_strict_tile_kernel launch failed: ", cudaGetErrorString(err));
  return out;
}

static torch::Tensor lynn_native_active_moe_scalar_contract_impl(
    torch::Tensor x,
    torch::Tensor expert_ids,
    torch::Tensor routing_weights,
    torch::Tensor gate_up_packed,
...
static torch::Tensor lynn_native_active_moe_strict_fused_boundary_impl(
    ...
    torch::Tensor down_scale,
    torch::Tensor down_global_scale) {
  auto inter = lynn_native_gate_up_silu_strict_tile_impl(
      x,
      expert_ids,
      gate_up_packed,
      gate_up_scale,
      gate_up_global_scale);
  return lynn_native_down_weighted_sum_strict_tile_impl(
      inter,
      expert_ids,
      routing_weights,
      down_packed,
...
torch::Tensor lynn_native_active_moe_scalar_contract(
    ...
    torch::Tensor down_scale,
    torch::Tensor down_global_scale) {
  return lynn_native_active_moe_scalar_contract_impl(
      x,
      expert_ids,
      routing_weights,
      gate_up_packed,
